...
function resolvePixelRatio(maxPixelRatio) {
  const devicePixelRatio = requirePositiveNumber(
...
export function renderToPngDataUrl(options) {
  ...
  if (
    ...
    throw new TypeError(
      "renderToPngDataUrl options.background must be a non-empty CSS color string when provided.",
    );
  }
...
export class CanvasSurface {
  constructor(options) {
    ...
    this._resizeTarget =
      settings.resizeTarget === undefined || settings.resizeTarget === null
    ...
    this._maxPixelRatio =
      settings.maxPixelRatio === undefined
        ? 3
  ...
  measure() {
    ...
    const changed =
      logicalSize.width !== this._size.width ||
  ...
  toPngDataUrl(options) {
    ...
    const settings =
      options === undefined ? {} : requireObject(options, "CanvasSurface toPngDataUrl options");
    ...
    const pixelRatio =
      settings.pixelRatio === undefined ? this._size.pixelRatio : settings.pixelRatio;
    const customDraw = readOptionalFunction(
      settings.draw,
      "CanvasSurface toPngDataUrl options.draw",
    );
    ...
    return renderToPngDataUrl({
      ...
      draw: customDraw
        ...
        : (context, exportSize) =>
            context.drawImage(this._canvas, 0, 0, exportSize.width, exportSize.height),
    });
  }
...
export class PointerRouter {
  constructor(options) {
    ...
    this._coordinateElement =
      settings.coordinateElement === undefined || settings.coordinateElement === null
        ? this._element
        : requireElement(settings.coordinateElement, "PointerRouter options.coordinateElement");
    this._onPointerDown = requireFunction(
      settings.onPointerDown,
      "PointerRouter options.onPointerDown",
    );
    this._onPointerMove = requireFunction(
      settings.onPointerMove,
      "PointerRouter options.onPointerMove",
    );
    this._onPointerUp = requireFunction(settings.onPointerUp, "PointerRouter options.onPointerUp");
    this._onPointerCancel = requireFunction(
    ...
    this._handleLostPointerCapture = (event) =>
      this._pointerCancel(event, "lostpointercapture", false);
  ...
  _pointerUp(event) {
    ...
    try {
      this._invokeCallback(
        this._onPointerUp,
        this._buildPayload("up", event, point),
        "onPointerUp",
      );
    } finally {
      this._releaseSession(true);
  ...
  _buildPayload(phase, event, point) {
    ...
    if (!session) {
      throw new Error(
        `PointerRouter cannot build a ${phase} payload without an active pointer session.`,
      );
    }
...
export function createHitRegionRegistry() {
