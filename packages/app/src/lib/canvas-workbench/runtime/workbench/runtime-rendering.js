import { resolveSurfaceMode } from "./runtime-interaction-router.js";
import { resolveLinkAnchorSides } from "./layout-and-legacy-render.js";
import {
  createElement,
  now,
...
import {
  getLinkAnchorPoint,
  getNodeRetainedContentKey,
  buildActiveDragContext,
  positionFloatingOverlayWithinHost,
  hidePopover,
  getConnectorAnchorPoints,
  worldToHostPoint,
} from "./layout-and-legacy-render.js";
import { setSelection, render } from "./interaction-and-state.js";
...
import {
  resolveCanvasNodeDetailMode,
  drawRoundedPanel,
  buildCanvasSnapshotBounds,
  reconcileRetainedLayer,
  renderGroupFrames,
  renderLinks,
  renderCanvasLinkLabels,
  renderCanvasMicroNode,
  renderCanvasInlineTextNode,
  renderCanvasDecisionNode,
  renderCanvasStandardNode,
  renderCanvasAdvancedNode,
} from "./canvas-renderers.js";

export function renderNodes(state, visibleNodes) {
...
export function renderActiveDrag(state) {
...
export function renderSnapGuides(state) {
  if (!state?.guideLayer) {
    return;
  }
...
export function renderConnectorAnchorOverlay(state, visibleNodes) {
  if (!state?.anchorLayer) {
    return;
  }
    ...
    for (const point of getConnectorAnchorPoints(state, node, anchors.placementMode)) {
      ...
      if (point.accentColor) {
        ...
        anchor.style.setProperty(
          "--cw-connector-anchor-shadow",
          hexToRgba(point.accentColor, 0.18),
        );
      }
...
export function renderTransformHandlesOverlay(state, visibleNodes) {
  if (!state?.transformLayer) {
    return;
  }
...
export function renderDebugDecorations(state, visibleNodes) {
  if (!state?.debugLayer) {
    return;
  }
...
export function renderMinimap(state, visibleNodes) {
...
export function measureRenderedNodeSizes() {
  return false;
}

export function scheduleNodeMeasurement() {}

export function applySceneTransform(state) {
  if (state?.scene) {
...
export function applyMarqueeSelection(state, interaction) {
...
export function buildDiagnosticsSnapshot(state, bounds) {
  return {
...
export function showPopover(state, anchor, annotation) {
  if (
