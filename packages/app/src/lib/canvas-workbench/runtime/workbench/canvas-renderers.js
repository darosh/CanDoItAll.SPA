import { getTextMeasureService, incrementMetric, round, getVisibleNodes } from "./foundation.js";
import {
  getLinkAnchorPoint,
  resolveCollapseAnchorInfo,
  getLinkRetainedKey,
  shouldRenderArrow,
  getExpandedFrameNodeIds,
  getFrameRetainedKey,
  getFrameBounds,
  resolveProgressDisplay,
  resolveMarkerGlyph,
  worldToHostPoint,
  resolveLinkAnchorSides,
} from "./layout-and-legacy-render.js";
import { render, resolveActionGlyph } from "./interaction-and-state.js";
import {} from "./context-menu-and-composer.js";
import {} from "./viewport-and-events.js";
...
} from "./canvas-scene-and-hit-testing.js";
function isActiveCanvasDrag(state) {
  const kind = state?.interaction?.kind || "";
  return kind === "drag" || kind === "frame-drag" || kind === "dependency-drag";
}

export function resolveCanvasNodeDetailMode(state, projectedNodeCount) {
  if (isActiveCanvasDrag(state)) {
    return (state.ui.zoom || 1) <= 0.9 || projectedNodeCount >= 24 ? "micro" : "compact";
  }
...
export function setCanvasFont(context, weight, sizePx) {
  context.font = `${weight} ${Math.max(8, round(sizePx))}px "DM Sans", "Segoe UI", sans-serif`;
}

export function drawCanvasTextLines(context, lines, x, startY, lineHeight, fillStyle) {
...
function getCachedCanvasTextLines(
  state,
  node,
  slot,
  text,
...
function wrapCanvasNodeText(state, node, slot, primitives, context, text, maxWidth, maxLines) {
  ...
  return getCachedCanvasTextLines(
    ...
    () => (primitives?.wrapText ? primitives.wrapText(context, text, maxWidth, maxLines) : [text]),
  );
}
...
function wrapCanvasInlineTextParagraphs(primitives, context, text, maxWidth, maxLines) {
  ...
  for (let index = 0; index < paragraphs.length; index += 1) {
    ...
    const normalizedLines =
      Array.isArray(wrappedLines) && wrappedLines.length > 0 ? wrappedLines : [paragraph];
...
export function drawRoundedPanel(context, bounds, radius, fill, stroke, lineWidth, shadowColor) {
  context.save();
  if (shadowColor) {
...
export function requestSceneImage(state, sourceUrl) {
  if (!sourceUrl) {
    return null;
  }
...
export function buildCanvasSnapshotBounds(bounds, node, extra) {
  return {
    id: node?.id || extra?.id || "",
...
export function reconcileRetainedLayer(
  retained,
  nextEntries,
  metrics,
  metricKey,
...
export function drawCanvasFrame(context, state, frame, hostBounds, memberCount, frameId) {
  ...
  const labelWidth = Math.min(
    hostBounds.width - 24,
    Math.max(92, (frame?.label || "Group border").length * 8 * state.ui.zoom + 52),
  );
  const labelBounds = buildRect(
    hostBounds.left + 18,
    hostBounds.top - labelHeight / 2,
    labelWidth,
    labelHeight,
  );
  drawRoundedPanel(
  ...
  context.fillText(
    frame?.label || "Group border",
    labelBounds.left + 12,
    labelBounds.top + Math.max(14, 18 * state.ui.zoom),
  );
  ...
  const handles = [
    buildRect(
      hostBounds.left + handleInsetX - handleSize / 2,
      hostBounds.top - handleSize / 2,
      handleSize,
      handleSize,
    ),
    buildRect(
      hostBounds.right - handleSize / 2,
      hostBounds.top + handleInsetY - handleSize / 2,
      handleSize,
      handleSize,
    ),
    buildRect(
      hostBounds.left + handleInsetX - handleSize / 2,
      hostBounds.bottom - handleSize / 2,
      handleSize,
      handleSize,
    ),
    buildRect(
      hostBounds.left - handleSize / 2,
      hostBounds.top + handleInsetY - handleSize / 2,
      handleSize,
      handleSize,
    ),
...
export function renderGroupFrames(state, visibleNodes) {
  ...
  for (const [index, frame] of (state.ui.groupFrames || []).entries()) {
    ...
    const labelBounds = drawCanvasFrame(
      surface.context,
      state,
      frame,
      hostBounds,
  ...
  switch ((port?.tone || "").toLowerCase()) {
    ...
    default:
      return {
        ...
        stroke:
          paletteStyle?.subtleStroke || resolved.portDefaultStroke || "rgba(148, 163, 184, 0.34)",
...
function sampleBezierTangent(startPoint, controlPoint1, controlPoint2, endPoint, t) {
  ...
  return {
    x:
      3 * inverseSquared * (controlPoint1.x - startPoint.x) +
      6 * inverse * t * (controlPoint2.x - controlPoint1.x) +
      3 * tSquared * (endPoint.x - controlPoint2.x),
    y:
      3 * inverseSquared * (controlPoint1.y - startPoint.y) +
  ...
  switch ((link?.tone || "").toLowerCase()) {
    ...
    default:
      return {
        ...
        stroke:
          fallbackStyle?.stroke || resolved.linkLabelDefaultStroke || "rgba(100, 116, 139, 0.32)",
...
function drawCanvasLinkLabel(context, link, geometry, fallbackStyle, colors) {
  ...
  const label = rawLabel.length > 32 ? `${rawLabel.slice(0, 29)}...` : rawLabel;
  ...
  const height = 24;
  const bounds = buildRect(labelPoint.x - width / 2, labelPoint.y - height / 2, width, height);
  drawRoundedPanel(
    context,
    bounds,
    12,
    ...
    style.stroke,
    1,
    colors?.linkLabelPanelShadow || "rgba(15, 23, 42, 0.1)",
  );
...
export function renderCanvasLinkLabels(context, renderedLinks, colors) {
...
export function drawCanvasLink(context, link, startPoint, endPoint, options) {
...
export function renderLinks(state, visibleNodes, options) {
  ...
  const linkFilter = typeof options?.linkFilter === "function" ? options.linkFilter : null;
  ...
  for (const [index, link] of state.surface.links.entries()) {
    ...
    const sourceAnchor = worldToHostPoint(
      state,
      getLinkAnchorPoint(
        state,
        source,
    ...
    const targetAnchor = worldToHostPoint(
      state,
      getLinkAnchorPoint(
        state,
        target,
  ...
  if (
    ...
    if (hoveredTargetId && state.lookups.byId.has(hoveredTargetId)) {
      ...
      const anchorSides = resolveLinkAnchorSides?.(
        state,
        previewSource,
        previewTarget,
        detailMode,
      ...
      const previewSourceBounds = projectSceneBounds(
        state,
        getNodeSceneBounds(state, previewSource),
      );
      const sourceSide = resolveCanvasAnchorSideFromHostBounds(
        previewSourceBounds,
  ...
  if (includePreviewLinks && state.connectionDraft) {
    const outputDescriptor =
      state.connectionDraft.direction === "input" ? state.connectionTarget : state.connectionDraft;
    const inputDescriptor =
      state.connectionDraft.direction === "input" ? state.connectionDraft : state.connectionTarget;
...
export function drawCanvasBadgePill(context, bounds, text, fill, stroke, textColor, fontSize) {
  ...
  context.fillText(
    ...
    bounds.top + bounds.height / 2 + fontSize * 0.34,
  );
  context.restore();
}

export function drawCanvasProgressBadge(context, state, bounds, node, paletteStyle) {
  const colors = state.colors || {};
  ...
  context.strokeStyle =
    paletteStyle?.progressTrack || colors.progressTrackDefault || "rgba(148, 163, 184, 0.28)";
  context.beginPath();
  context.arc(centerX, centerY, radius - context.lineWidth, 0, Math.PI * 2);
  context.stroke();
  if (display.mode !== "na") {
    context.strokeStyle =
      display.mode === "complete"
    ...
    context.arc(
      ...
      -Math.PI / 2,
      -Math.PI / 2 + (display.angle / 360) * Math.PI * 2,
    );
    context.stroke();
  }
  ...
  context.fillStyle =
    paletteStyle?.progressText || colors.progressTextDefault || "rgba(15, 23, 42, 0.78)";
  context.textAlign = "center";
...
function drawCanvasMarkerBadges(
  context,
  state,
  node,
  accent,
  ...
  for (const marker of markers.slice(0, limit)) {
    ...
    cursor = direction === "left" ? bounds.left - badgeGap : bounds.right + badgeGap;
  }
...
export function drawCanvasAnnotationBadges(
  context,
  state,
  node,
  startX,
  ...
  for (let index = 0; index < annotations.length; index += 1) {
    ...
    const width = Math.min(
      maxWidth,
      Math.max(36, context.measureText(label).width + 16 * state.ui.zoom),
    );
...
export function drawNodeMediaPreview(context, state, node, bounds, radius) {
  ...
  context.fillText(
    node.mediaKind === "image" ? "Image" : "Preview",
    bounds.left + 12 * state.ui.zoom,
    bounds.bottom - 12 * state.ui.zoom,
  );
  context.restore();
  return true;
}
...
function isWorkflowDecisionNode(node) {
  ...
  return family === "workflow-decision" || paletteKey === "workflow-decision";
}
...
function drawCanvasDecisionCues(context, state, node, hostBounds, paletteStyle) {
  ...
  context.fillStyle =
    paletteStyle.iconFill || colors.decisionCueIconFillDefault || "rgba(255, 251, 235, 0.9)";
  context.strokeStyle =
    paletteStyle.iconStroke || colors.decisionCueIconStrokeDefault || "rgba(245, 158, 11, 0.56)";
...
export function renderCanvasDecisionNode(
  context,
  state,
  node,
  hostBounds,
  ...
  context.shadowColor = isSelected
    ? colors.decisionShadowSelected || "rgba(15, 118, 110, 0.2)"
    : colors.decisionShadowDefault || "rgba(15, 23, 42, 0.12)";
  ...
  for (const anchor of anchors) {
    ...
    context.strokeStyle =
      accent || colors.decisionAnchorStrokeDefault || "rgba(20, 184, 166, 0.92)";
    context.stroke();
    context.restore();
  ...
  if ((node.branchLabel || "").trim() && detailMode !== "compact") {
    ...
    const labelWidth = Math.min(
      radius * 1.45,
      Math.max(74, node.branchLabel.length * 6.5 * zoom + 22),
    );
    drawCanvasBadgePill(
      context,
      buildRect(
...
function drawCanvasBranchLabelPill(
  context,
  state,
  node,
  startX,
...
export function renderCanvasMicroNode(context, state, node, hostBounds, accent, meta) {
  const isSelected = state.selectedIds.has(node.id);
...
export function renderCanvasInlineTextNode(
  ...
  const reservedBottom =
    Math.max(26, 30 * state.ui.zoom) + (metaRows - 1) * Math.max(12, 16 * state.ui.zoom);
  const maxTextLines =
    detailMode === "compact"
      ? 2
      : Math.max(
          2,
          Math.floor(
            Math.max(lineHeight, hostBounds.height - padding * 2 - reservedBottom) / lineHeight,
          ),
        );
  ...
  badgeLeft = drawCanvasMarkerBadges(
    context,
    state,
    node,
    accent,
...
export function getCanvasAdvancedNodePortLayout(state, node, hostBounds, detailMode) {
  const zoom = Math.max(state?.ui?.zoom || 1, 0.01);
  const resolvedDetailMode =
    detailMode || (zoom <= 0.3 ? "micro" : zoom <= 0.55 ? "compact" : "full");
  ...
  const columnWidth = hasBothSides ? Math.max(72, (contentWidth - columnGap) / 2) : contentWidth;
  const inputColumnLeft = contentLeft;
  const outputColumnLeft = hasBothSides ? hostBounds.right - padding - columnWidth : contentLeft;
  ...
  const supportingTop =
    titleTop + titleLines.length * titleLineHeight + titleGap + Math.max(3, 5 * zoom);
  ...
  if (requiredHeight > availableHeight && minimumRowCount > 1) {
    portGap = Math.max(
      1.5 * zoom,
      (availableHeight - minimumRowCount * portHeight) / (minimumRowCount - 1),
    );
    requiredHeight = minimumRowCount * portHeight + Math.max(0, minimumRowCount - 1) * portGap;
    if (requiredHeight > availableHeight) {
      portHeight = Math.max(
        14,
        (availableHeight - Math.max(0, minimumRowCount - 1) * Math.max(1.5 * zoom, portGap)) /
          minimumRowCount,
      );
      portGap = Math.max(
        1.5 * zoom,
        (availableHeight - minimumRowCount * portHeight) / Math.max(1, minimumRowCount - 1),
      );
    }
  }

  const buildPortEntries = (ports, side, columnLeft) =>
...
function resolveConnectionAnchorHostPoint(state, descriptor, fallbackDirection) {
  ...
  const anchorSide = descriptor.side || (descriptor.direction === "input" ? "left" : "right");
  const anchorPoint = worldToHostPoint(
    ...
    getLinkAnchorPoint(
      ...
      descriptor.direction ||
        fallbackDirection ||
        (anchorSide === "left" || anchorSide === "top" ? "input" : "output"),
    ),
  );
...
export function renderCanvasAdvancedNode(
  context,
  state,
  node,
  hostBounds,
  ...
  context.fillRect(
    hostBounds.left + 8,
    hostBounds.top + 8,
    Math.max(6, 8 * state.ui.zoom),
    Math.max(30, hostBounds.height - 16),
  );
  context.restore();
  ...
  context.fillText(
    ...
    layout.contentLeft + Math.max(20, 26 * state.ui.zoom),
  ...
  if (outputPorts.length > 0) {
    ...
    context.fillText(
      "OUTPUTS",
      layout.outputColumnLeft + layout.columnWidth,
  ...
  const footerPillBounds = buildRect(
    layout.contentLeft,
    layout.footerTop,
    Math.max(74, 88 * state.ui.zoom),
    layout.footerHeight,
  );
  drawCanvasBadgePill(
    context,
...
export function renderCanvasStandardNode(
  context,
  state,
  node,
  hostBounds,
  ...
  if (node.isPreviewOnly) {
    context.save();
    context.strokeStyle =
      (state.colors || {}).standardNodePreviewOutlineStroke || "rgba(14, 165, 233, 0.38)";
    ...
    context.roundRect(
      hostBounds.left + 6,
      hostBounds.top + 6,
      Math.max(0, hostBounds.width - 12),
      Math.max(0, hostBounds.height - 12),
  ...
  context.fillText(
    node.kind || node.family || "item",
    contentLeft + Math.max(20, 26 * state.ui.zoom),
    cursorY + Math.max(7, 10 * state.ui.zoom),
  );
  drawCanvasBadgePill(
    context,
  ...
  rightCursor = drawCanvasMarkerBadges(
    context,
    state,
    node,
    accent,
  ...
  if (node.compactPath?.fullPath && detailMode !== "micro") {
    ...
    context.fillText(
      pathLabel,
      pathBounds.left + Math.max(8, 10 * state.ui.zoom),
      pathBounds.top + Math.max(13, 15 * state.ui.zoom),
    );
    context.textAlign = "right";
    context.fillText(
      state.pathCopyState?.nodeId === node.id
        ? resolveActionGlyph("qa")
