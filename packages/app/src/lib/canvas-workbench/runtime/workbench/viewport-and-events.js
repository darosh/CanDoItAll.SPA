...
import { syncWorkbenchMode, openNodeMetadataMenu } from "./runtime-interaction-router.js";
import {
  applyViewportPreviewTransform,
  cancelDeferredViewportRender,
  scheduleDeferredViewportRender,
  flushDeferredViewportRender,
  clearNodeHighlights,
} from "./interaction-and-state.js";
import { routeContextMenuShortcut } from "./context-menu-shortcuts.js";
import { resize } from "./runtime-entry.js";
import {
  MIN_ZOOM,
  MAX_ZOOM,
  getViewportControllerService,
  getAnimationTimelineService,
  clamp,
  debounce,
...
import {
  ...
  layoutComposer,
  render,
  syncContextMenuLayers,
} from "./interaction-and-state.js";
import {
  openKeyboardNoteComposer,
  openExistingNoteEditor,
  showContextMenu,
} from "./context-menu-and-composer.js";
...
export function isMarqueeModifierPressed(state, event) {
...
export function startMarquee(state, event) {
...
export function ensureSelectedForDrag(state, nodeId) {
  if (!state.selectedIds.has(nodeId)) {
    selectSingleNode(state, nodeId);
  }
}

export function startDragForNodeIds(state, event, nodeIds, options) {
...
export function startDrag(state, event, nodeId) {
  ensureSelectedForDrag(state, nodeId);
  startDragForNodeIds(state, event, [...state.selectedIds], { kind: "drag" });
}

export function startFrameDrag(state, event, frameId) {
...
export function updateMarquee(state, event) {
  const point = getHostPoint(state, event.clientX, event.clientY);
...
export function legacyApplyMarqueeSelection(state) {
...
export function updateDrag(state, event) {
  ...
  state.interaction.moved =
    state.interaction.moved || Math.abs(deltaX) > 0.5 || Math.abs(deltaY) > 0.5;
...
export function updatePan(state, event) {
  ...
  setPan(state, state.interaction.panX + deltaX, state.interaction.panY + deltaY, state.ui.zoom, {
    skipClamp: true,
  });
  applyViewportPreviewTransform?.(state);
}

export async function finishInteraction(state) {
  ...
  switch (interaction.kind) {
    ...
    case "marquee":
      state.interaction = null;
      (applyMarqueeSelection || legacyApplyMarqueeSelection)(state, interaction);
      break;
    default:
      state.interaction = null;
      break;
...
function getFloatingWindowOccluders(state, hostRect) {
  const scope =
    state.host?.closest?.(".cw-stage-surface") ||
    state.shell ||
    state.host?.parentElement ||
    state.document;
  ...
  return Array.from(scope.querySelectorAll(".cw-floating-window"))
    ...
    .map((candidate) => {
      ...
      return localRect.width > 1 && localRect.height > 1 ? localRect : null;
    })
...
function getPreferredViewportRect(state, margin) {
  ...
  for (const occluder of occluders) {
    candidates.push(
      buildViewportRect(hostViewport.left, hostViewport.top, hostViewport.right, occluder.top),
    );
    candidates.push(
      buildViewportRect(
        hostViewport.left,
    ...
    candidates.push(
      buildViewportRect(hostViewport.left, hostViewport.top, occluder.left, hostViewport.bottom),
    );
    candidates.push(
      buildViewportRect(occluder.right, hostViewport.top, hostViewport.right, hostViewport.bottom),
    );
  }
...
function applyViewportMargin(viewportRect, margin) {
  ...
  return insetViewport.width > 0 && insetViewport.height > 0 ? insetViewport : viewportRect;
}

export function isNodeVisibleInViewport(state, node, margin) {
  const viewportRect = getPreferredViewportRect(state, margin);
  const renderedNode = getRenderedNodeBounds(state, node.id);
  if (renderedNode) {
    return (
      renderedNode.left >= viewportRect.left &&
  ...
  return (
    position.x - halfWidth >= viewportRect.left &&
...
export function centerNodeElementInViewport(state, nodeId) {
...
export function ensureNodeVisible(state, nodeId, options) {
...
export function legacyResize(state) {
...
export function findContainingBlockOverrides(state) {
  const overrides = [];
...
export function suspendContainingBlock(state) {
...
export function restoreContainingBlock(state) {
  if (!state.containingBlockOverride) {
    return;
  }
...
export function setMaximized(state, isMaximized, options) {
  ...
  const resizeSurface = resize || legacyResize;
  resizeSurface(state);
  render(state);
...
export function applyWheelZoom(state, event) {
...
export function setZoomPercent(state, percent, anchorPoint, options) {
  cancelViewportAnimation(state);
  const rect = state.host.getBoundingClientRect();
  ...
  setPan(state, target.panX, target.panY, target.zoom);
  if (options?.renderMode === "animation-frame") {
    applyViewportPreviewTransform?.(state);
    scheduleDeferredViewportRender?.(state, options?.delayMs ?? 180);
  } else {
    render(state);
  }
...
export function setMenuScalePercent(state, menuScalePercent) {
...
export function isManualDoubleActivation(state, nodeId) {
  const now = Date.now();
...
export function handleNodeDoubleActivation(state, node) {
...
export async function requestNodeOpen(state, nodeId, options) {
  const node = state?.lookups?.byId?.get?.(nodeId);
...
export function legacyAttachEvents(state) {
  state.handlers = {
    pointerDown: (event) => {
      ...
      flushDeferredViewportRender?.(state);
    ...
    contextMenu: (event) => {
      ...
      const isGroupSelection =
        !!targetNode && state.selectedIds.size > 1 && state.selectedIds.has(targetNode.id);
    ...
    keyDown: (event) => {
      ...
      if (routeContextMenuShortcut?.(state, event)) {
        return;
      }
      ...
      const isClipboardShortcut =
        lowerKey === "x" || lowerKey === "c" || lowerKey === "v" || lowerKey === "d";
      ...
      switch (event.key) {
        ...
        case "Escape":
          event.preventDefault();
          {
            ...
            clearContextMenu(state);
            closeComposer(state);
            clearNodeHighlights?.(state, { render: true, publish: true });
...
export function hydrateState(host, dotNetRef, surface, selectionDispatchSeed, stateDispatchSeed) {
  ...
  const state = {
    ...
    metrics: createWorkbenchMetrics(),
    publishStateDebounced: debounce(
      (stateJson) => invokeStateChanged(state, stateJson, "debounced"),
      140,
    ),
  };
...
export function refresh(state, surface, isMaximized, preserveViewport) {
  cancelViewportAnimation(state);
  ...
  setMaximized(state, typeof isMaximized === "boolean" ? isMaximized : !!state.ui.isMaximized);
  resize(state);
  const incomingViewportIsDefault =
    Math.abs(incomingViewport.panX - 90) <= 0.5 &&
    Math.abs(incomingViewport.panY - 110) <= 0.5 &&
    Math.abs(incomingViewport.zoom - 1) <= 0.001;
  const shouldPreserveViewport =
    preserveViewport ||
    incomingViewportIsDefault ||
    (Math.abs(incomingViewport.panX - previousViewport.panX) <= 0.5 &&
  ...
  const shouldRevealSelection =
    !preserveViewport && !!selectedNodeId && (!!pendingCreate || selectionChanged);
  const shouldRestoreVisibleSelection =
    !preserveViewport &&
    !!selectedNodeId &&
    !!selectedNode &&
  ...
  if (shouldRevealSelection || shouldRestoreVisibleSelection) {
    ...
    ensureNodeVisible(state, selectedNodeId, {
      forceCenter: isNewNode || shouldRestoreVisibleSelection,
    });
  }
