import { finishInteraction } from "./viewport-and-events.js";
import { requestClipboardPaste } from "./layout-and-legacy-render.js";
import { routeContextMenuShortcut } from "./context-menu-shortcuts.js";
import { flushDeferredViewportRender, clearNodeHighlights } from "./interaction-and-state.js";
import { clamp, cancelViewportAnimation } from "./foundation.js";
import {
  invokeAnnotationAction,
  showStatusNotice,
  writeClipboardText,
...
import {
  ...
  openContextSubmenuByActionId,
  showContextMenu,
} from "./context-menu-and-composer.js";
import {
  startPan,
  isMarqueeModifierPressed,
  startMarquee,
  startDragForNodeIds,
  startDrag,
  startFrameDrag,
  ...
  setZoomPercent,
  toggleHelp,
  isManualDoubleActivation,
  handleNodeDoubleActivation,
} from "./viewport-and-events.js";
import {
  resolveAnchorRect,
  getSceneHitAtPoint,
  getSceneHitAtEvent,
...
import {} from "./canvas-renderers.js";

function finishCanvasInteraction(state) {
  return finishInteraction(state);
}

export function openNodeMetadataMenu(state, node, actionId, anchor) {
  if (!node || !anchor) {
    return;
  }
...
export async function copyCompactPath(state, button, compactPath) {
...
export function resolveSurfaceMode(state) {
  return state?.surface?.mode || "authoring";
}

export function isDeleteMode(state) {
  return resolveSurfaceMode(state) === "delete";
}

export function isDependencyMode(state) {
  return resolveSurfaceMode(state) === "dependency";
}

export function syncWorkbenchMode(state) {
...
export function setWorkbenchToolMode(state, mode) {
  if (!state?.surface) {
    return;
  }

  const normalizedMode = mode === "delete" || mode === "dependency" ? mode : "authoring";
...
export function updatePointerHostPoint(state, event) {
  ...
  state.pointerHostPoint = isWithinHost ? getHostPoint(state, event.clientX, event.clientY) : null;
  return state.pointerHostPoint;
}

export function dispatchContextActionRequest(state, request) {
...
export function distancePointToSegment(pointX, pointY, startX, startY, endX, endY) {
...
export function cubicBezierPoint(link, t) {
  const inverse = 1 - t;
...
export function isPointNearRenderedLink(link, pointX, pointY) {
  if (!link?.bounds) {
    return false;
  }
  ...
  const margin = 12;
  if (
    pointX < link.bounds.left - margin ||
  ...
  for (let index = 1; index <= segments; index += 1) {
    ...
    if (
      distancePointToSegment(pointX, pointY, previous.x, previous.y, current.x, current.y) <= 10
    ) {
      return true;
    }
...
export function hitTestRenderedLink(state, pointX, pointY) {
...
export function resolveDeleteModeHitTarget(state, event) {
...
export function updateDeleteHoverState(state, event) {
  ...
  if (
    (state.hoveredDeleteNodeId || null) === nextNodeId &&
...
export function supportsConnectionAuthoring(state) {
  return resolveSurfaceMode(state) === "authoring";
}

export function resolveConnectorAnchorHit(event) {
...
export function resolveConnectionPortHit(state, event, sceneHit) {
  ...
  return hitTarget?.type === "node-port" ? hitTarget : null;
}

export function buildConnectionDescriptor(portHit) {
  if (!portHit?.nodeId) {
    return null;
  }
...
export function clearConnectionDraft(state, shouldRender = true) {
...
export function startConnectionDraft(state, descriptor) {
...
export function updateConnectionDraftTarget(state, event, sceneHit) {
...
export function dispatchConnectionCreate(state, targetDescriptor) {
  if (!state.connectionDraft || !targetDescriptor) {
    return false;
  }

  const sourceDescriptor =
    state.connectionDraft.direction === "input" ? targetDescriptor : state.connectionDraft;
  const inputDescriptor =
    state.connectionDraft.direction === "input" ? state.connectionDraft : targetDescriptor;
...
export function attachEvents(state) {
  state.handlers = {
    pointerDown: (event) => {
      ...
      if (state.connectionDraft && event.button === 0) {
        ...
        if (
          ...
          portDescriptor.direction ===
            resolveOppositeConnectionDirection(state.connectionDraft.direction) &&
      ...
      if (
        supportsConnectionAuthoring(state) &&
        event.button === 0 &&
        portDescriptor &&
        (portDescriptor.direction === "output" || portDescriptor.direction === "input")
      ) {
        ...
        if (
          ...
          if (hitTarget?.type === "node-progress") {
            ...
            openNodeMetadataMenu(
              ...
              targetNode,
              "progress",
              resolveAnchorRect(hitTarget.bounds),
            );
            return;
          }
    ...
    keyDown: (event) => {
      ...
      if (
        ...
        const selectedNodeId =
          state.ui?.selectedNodeIds?.length === 1 ? state.ui.selectedNodeIds[0] : null;
