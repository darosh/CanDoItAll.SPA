...
import { setCanvasFont, drawRoundedPanel } from "./canvas-renderers.js";

import { buildDiagnosticsSnapshot } from "./runtime-rendering.js";
import {
  copyCompactPath,
  resolveSurfaceMode,
  syncWorkbenchMode,
  attachEvents,
} from "./runtime-interaction-router.js";
...
export function buildWorkbench(state) {
  clear(state.host);
  ...
  const frameCanvas = createElement(
    state.document,
    "canvas",
    "cw-workbench__canvas cw-workbench__canvas--frames",
  );
  const linkCanvas = createElement(
    state.document,
    "canvas",
    "cw-workbench__canvas cw-workbench__canvas--links",
  );
  const nodeCanvas = createElement(
    state.document,
    "canvas",
    "cw-workbench__canvas cw-workbench__canvas--nodes",
  );
  ...
  const diagnosticsTitle = createElement(
    state.document,
    "p",
    "cw-diagnostics__title",
    "Diagnostics",
  );
...
export function drawEmptyStateToExport(context, width, height, state) {
  ...
  context.fillText(
    state.surface.chrome.emptyStateKicker || "Canvas",
    bounds.left + 16,
    bounds.top + 22,
  );
  ...
  context.fillText(
    state.surface.chrome.emptyStateTitle || "No nodes yet",
    bounds.left + 16,
    bounds.top + 48,
  );
...
export function collectSceneSnapshot(state) {
  return {
    rendererMode: "canvas",
...
export function findSceneHotZoneCenter(state, request) {
  ...
  for (let index = hotZones.length - 1; index >= 0; index -= 1) {
    ...
    if (
      (!zoneType || entry.metadata?.type === zoneType) &&
...
export function activateHotZone(state, request) {
...
export function createSyntheticPointerEvent(state, clientX, clientY, request) {
  return {
    clientX,
    clientY,
...
export function resolveSyntheticNodeDragStart(state, request) {
...
export function resolveSyntheticDragStart(state, request) {
  if (request?.frameId) {
...
export function simulatePointerDrag(state, request) {
  if (
...
export function releaseSyntheticInteraction(state) {
...
function disposeWorkbenchStateCore(state, options) {
  ...
  setMaximized(state, false, { render: false });
  clear(state.host);
  delete state.host.__canvasWorkbenchState;
}

export function legacyDisposeWorkbenchState(state) {
  if (!state) {
    return;
  }
...
export function disposeWorkbenchState(state) {
  if (!state) {
    return;
  }
...
function applyRenderOptions(state, options) {
  ...
  if (Array.isArray(options.selectedNodeIds) && options.selectedNodeIds.length > 0) {
    applyRequestedSelection(
      ...
      options.primaryNodeId || options.selectedNodeIds[0],
    );
  }
}

function resolveMaximizedState(state, options) {
  return typeof options?.isMaximized === "boolean" ? options.isMaximized : !!state.ui.isMaximized;
}
...
export const canvasWorkbench = {
  create(host, dotNetRef, surface, selectionDispatchSeed, stateDispatchSeed, options) {
    ...
    attachEvents(state);
    setMaximized(state, resolveMaximizedState(state, options));
    if (typeof window.ResizeObserver === "function") {
      state.resizeObserver = new window.ResizeObserver(() => scheduleResizeObserverRender(state));
      const resizeTargets = [host, state.shell, host.closest(".cw-stage-surface")].filter(
    ...
    state.themeWatcher = themeTokens.watchTheme(state.shell || host, () => {
      if (state.isDisposed) {
        return;
      }
  ...
  update(host, surface, options) {
    ...
    const requestedIsMaximized =
      typeof options?.isMaximized === "boolean"
        ? options.isMaximized
        : !!surface?.uiState?.isMaximized;
    refresh(state, surface, requestedIsMaximized, options?.preserveViewport === true);
    applyRenderOptions(state, options);
    return true;
  },
  fitView(host) {
  ...
  getDiagnostics(host) {
    const state = resolveWorkbenchState(host, { requireConnected: false });
    return state ? buildDiagnosticsSnapshot(state, getSceneBounds(state)) : null;
  },
  getViewportSnapshot() {
    return {
  ...
  dispose(host) {
    ...
    disposeWorkbenchState(state);
  },
};

canvasWorkbench.getSceneSnapshot = function (host) {
  const state = resolveWorkbenchState(host, { requireConnected: false });
  return state ? collectSceneSnapshot(state) : null;
};

canvasWorkbench.getHotZoneCenter = function (host, request) {
  const state = resolveWorkbenchState(host, { requireConnected: false });
