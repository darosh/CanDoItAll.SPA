import { clear, createElement, round, getNodeSize, getNodePosition } from "./foundation.js";
import { getHostPoint, getWorldPoint } from "./layout-and-legacy-render.js";
import {
  setSelection,
  clearContextMenu,
  closeComposer,
  ...
  applyProgressPresetTone,
  fitContextMenuLabel,
  resolveActionGlyph,
  createMenuActionIcon,
  resolveContextMenuOffsets,
  getContextMenuLayerBounds,
  positionContextMenu,
  getContextMenuOrbitRadius,
  closeContextMenuLayersFrom,
  resolveSubmenuOrigin,
  cancelPendingContextSubmenu,
  scheduleContextSubmenuOpen,
  clampLayerOriginToHost,
  getToolboxPanelSize,
  clampToolboxPanelOriginToHost,
  resolveToolboxPanelOrigin,
  createContextMenuLayer,
  syncContextMenuLayerShellGeometry,
  nudgeContextMenuLayerIntoVisibleHost,
  submitCreateRequest,
  submitNodeEdit,
...
} from "./context-menu-shortcuts.js";
...
export function commitComposer(state) {
  if (!state.composer) {
    return;
  }
  ...
  if (state.composer.kind === "create") {
    ...
    const hasMissingRequiredInputs =
      Array.isArray(state.composer.inputFieldEntries) &&
      state.composer.inputFieldEntries.some(
        (entry) => entry.isRequired && !(entry.input.value || "").trim(),
      );
...
export function decorateComposerShell(state, title, kicker, variant) {
  const composer = createElement(
    state.document,
    "div",
    `cw-canvas-composer ${variant ? `is-${variant}` : ""}`,
  );
...
export function createComposerWizard(document, steps) {
  ...
  for (const step of steps) {
    ...
    item.appendChild(
      createElement(document, "span", "cw-canvas-composer__wizard-step", `Step ${step.number}`),
    );
    item.appendChild(
      createElement(document, "strong", "cw-canvas-composer__wizard-label", step.title),
    );
...
export function createComposerSection(document, stepNumber, title, description) {
  ...
  header.appendChild(
    createElement(
      document,
      "span",
      "cw-canvas-composer__section-step",
...
export function updateComposerFileState(composer) {
  if (!composer) {
    return;
  }
  ...
  if (composer.createButton) {
    const hasMissingRequiredInputs =
      Array.isArray(composer.inputFieldEntries) &&
      composer.inputFieldEntries.some(
        (entry) => entry.isRequired && !(entry.input.value || "").trim(),
      );
    composer.createButton.disabled =
      (!!composer.requiresFile && !composer.uploadedFile) || hasMissingRequiredInputs;
  }
}

export function openCreateComposer(state, action, request) {
  clearContextMenu(state);
  closeComposer(state, { focusHost: false });

  const shell = decorateComposerShell(
    state,
    `Create ${action.label || "item"}`,
    action.label || "Create",
    "dialog",
  );
  const setupRendererKey = (action.setupRendererKey || "")
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9_-]/g, "-");
  if (setupRendererKey) {
  ...
  if (action.description) {
    overview.appendChild(
      createElement(state.document, "p", "cw-canvas-composer__intro", action.description),
    );
  }

  if (showDefaultTextFields) {
    const detailsSection = appendSection(
      "Details",
      "Name the item and keep the create request readable on the canvas.",
    );
    ...
    titleField.appendChild(
      createElement(state.document, "span", null, action.titleLabel || "Title"),
    );
    ...
    subtitleField.appendChild(
      createElement(state.document, "span", null, action.subtitleLabel || "Subtitle"),
    );
    ...
    notesField.appendChild(
      createElement(state.document, "span", null, action.notesLabel || "Notes"),
    );
  ...
  const initialInputValues =
    Array.isArray(request?.inputValues) && request.inputValues.length
      ? request.inputValues
      : Array.isArray(action?.defaultInputValues)
        ? action.defaultInputValues
        : [];
  ...
  const resolveInputFieldContainer = (field) => {
    ...
    const section = appendSection(
      ...
      field.sectionDescription ||
        "Complete the typed fields required before the item can be created.",
    );
  ...
  if (action.requiresFile) {
    ...
    const uploadTitle = createElement(
      state.document,
      "span",
      "cw-canvas-composer__upload-title",
      action.filePrompt || "Drop a file here or choose one.",
    );
    ...
    const fileTrigger = createElement(
      state.document,
      "button",
      "cw-button cw-canvas-composer__file-trigger",
      "Choose file",
    );
    fileTrigger.type = "button";
    fileTrigger.dataset.tone = "ghost";
    const dropCopy = createElement(
      state.document,
      "span",
      null,
      action.supportsDragDrop ? "Click or drop a file here." : "Click to choose a file.",
    );
    ...
    dropZone.addEventListener("click", (event) => {
      if (
        ...
        event.target?.closest?.(".cw-canvas-composer__file-trigger")
      ) {
        return;
      }
...
export function openInlineNoteComposer(state, options) {
  closeComposer(state, { focusHost: false });
  clearContextMenu(state);
  ...
  textInput.placeholder =
    options.placeholder || state.surface.chrome.inlineNotePlaceholder || "Write note";
  noteEditor.appendChild(textInput);
  noteEditor.appendChild(
    createElement(
      state.document,
      "p",
      "cw-note-editor__hint",
...
export function buildChildNotePlacement(position, childCount, sourceSize, horizontalDirection) {
...
export function buildSiblingNotePlacement(position, siblingCount, sourceSize) {
...
export function openKeyboardNoteComposer(state, placementKind) {
  ...
  const actionId = isSibling
    ? state.surface.chrome.siblingNoteActionId
    : state.surface.chrome.childNoteActionId;
  if (!actionId) {
    return;
  }
  ...
  const anchorWorld = isSibling
    ? buildSiblingNotePlacement(
        position,
        state.surface.nodes.filter(
          (candidate) => candidate.parentId === node.parentId && candidate.id !== node.id,
        ).length,
        sourceSize,
      )
    : buildChildNotePlacement(
        position,
        state.surface.nodes.filter((candidate) => candidate.parentId === node.id).length,
        sourceSize,
        resolveChildNoteHorizontalDirection(state, node),
      );
  ...
  openInlineNoteComposer(state, {
    ...
    value: "",
    placeholder:
      node.inlineTextPlaceholder || state.surface.chrome.inlineNotePlaceholder || "Write note",
  });
}

export function openExistingNoteEditor(state, node) {
  openInlineNoteComposer(state, {
    kind: "note-edit",
    nodeId: node.id,
...
export function executeContextAction(state, node, action, clientX, clientY, placementKind) {
...
export function openContextSubmenu(state, parentLayer, options, action, offset) {
  ...
  const submenuOffsets = resolveContextMenuOffsets(
    state,
    action.children || [],
    submenuBaseRadius,
    submenuRingStep,
...
export function openContextSubmenuByActionId(state, actionId) {
...
export function renderContextMenuLayer(state, layerState, options) {
  const layout = options.submenuLayout || "";
  const offsets =
    options.offsets ||
    resolveContextMenuOffsets(state, options.actions, options.baseRadius, options.ringStep, layout);
  ...
  layerState.bounds = getContextMenuLayerBounds(
    state,
  ...
  options.actions.forEach((action, index) => {
    ...
    button.dataset.shortcutKey = getMenuActionShortcutKey?.(action) || "";
    ...
    button.addEventListener("pointerleave", () => {
      cancelPendingContextSubmenu(
        ...
        (pending) =>
          ...
          pending.ownerDepth === layerState.depth,
      );
    });
    ...
    if (variant !== "priority-preset") {
      label =
        typeof buildContextMenuActionLabel === "function"
          ? buildContextMenuActionLabel(state, action)
          : createElement(
              state.document,
              "strong",
              "cw-context-menu__label",
              resolveMenuLabel(action),
            );
...
export function renderToolboxPreview(
  state,
  layerState,
  previewHost,
  action,
  ...
  const kicker = createElement(
    state.document,
    "span",
    "cw-context-toolbox__preview-kicker",
    `${sectionLabel} / ${groupLabel}`,
  );
  const title = createElement(
    state.document,
    "strong",
    "cw-context-toolbox__preview-title",
    action?.label || action?.menuLabel || "Component",
  );
...
export function renderToolboxPanelLayer(state, layerState, options) {
  ...
  headerCopy.appendChild(
    createElement(
      state.document,
      "strong",
      "cw-context-toolbox__title",
      options.label || "Prompt components",
    ),
  );
  if (options.description) {
    headerCopy.appendChild(
      createElement(state.document, "p", "cw-context-toolbox__copy", options.description),
    );
  }
  ...
  header.appendChild(
    createElement(
      state.document,
      "span",
      "cw-context-toolbox__count",
      `${options.actions?.length || 0} sections`,
    ),
  );
  ...
  const renderSections = () => {
    ...
    for (let sectionIndex = 0; sectionIndex < (options.actions || []).length; sectionIndex++) {
      ...
      const sectionSummary = createElement(
        state.document,
        "summary",
        "cw-context-toolbox__section-summary",
      );
      const sectionBadge = createElement(
        state.document,
        `span`,
        `cw-context-toolbox__section-badge tone-${sectionAction.tone || "neutral"}`,
      );
      sectionBadge.textContent = resolveActionGlyph(sectionAction.icon || "");
      const sectionSummaryCopy = createElement(
        state.document,
        "span",
        "cw-context-toolbox__section-copy",
      );
      sectionSummaryCopy.appendChild(
        createElement(state.document, "strong", null, sectionAction.label || "Section"),
      );
      sectionSummaryCopy.appendChild(
        createElement(state.document, "small", null, sectionAction.description || ""),
      );
      ...
      for (let groupIndex = 0; groupIndex < matchingGroups.length; groupIndex++) {
        ...
        const groupSummary = createElement(
          state.document,
          "summary",
          "cw-context-toolbox__group-summary",
        );
        groupSummary.appendChild(
          createElement(state.document, "strong", null, groupEntry.groupAction.label || "Group"),
        );
        groupSummary.appendChild(
          createElement(state.document, "small", null, groupEntry.groupAction.description || ""),
        );
        ...
        for (const itemAction of groupEntry.matchingItems) {
          ...
          item.addEventListener("pointerenter", () => {
            renderToolboxPreview(
              ...
              previewHost,
              itemAction,
              sectionAction.label || "Section",
              groupEntry.groupAction.label || "Group",
            );
          });
          ...
          itemBody.appendChild(
            createElement(
              ...
              "strong",
              null,
              itemAction.label || itemAction.menuLabel || "Item",
            ),
          );
          itemBody.appendChild(
            createElement(
              ...
              "small",
              null,
              itemAction.requiresInput
                ? "Specify inputs before inserting"
                : "Add directly to the prompt flow",
            ),
          );
    ...
    if (!sectionsHost.childElementCount) {
      sectionsHost.appendChild(
        createElement(
          state.document,
          "div",
          "cw-context-toolbox__empty",
          "No prompt components match the current search.",
        ),
      );
    ...
    if (firstPreview && layerState.previewActionId !== firstPreview.action.actionId) {
      renderToolboxPreview(
        ...
        previewHost,
        firstPreview.action,
        firstPreview.sectionLabel,
        firstPreview.groupLabel,
      );
    }
  };
...
export function showContextMenu(state, options) {
  clearContextMenu(state);
  ...
  const rootOffsets = resolveContextMenuOffsets(
    state,
    actions,
    84 * menuScale,
    62 * menuScale,
...
export function legacyOpenNodeMetadataMenu(state, node, actionId, anchorElement) {
