import {
  render,
  publishState,
  applyFullTextTooltip,
  resolveActionGlyph,
  toggleCollapse,
} from "./interaction-and-state.js";
import {
  renderConnectorAnchorOverlay,
  measureRenderedNodeSizes as runtimeMeasureRenderedNodeSizes,
  showPopover,
  buildDiagnosticsSnapshot as runtimeBuildDiagnosticsSnapshot,
} from "./runtime-rendering.js";
import { openNodeMetadataMenu } from "./runtime-interaction-router.js";
import { hexToRgba } from "./canvas-scene-and-hit-testing.js";
import { getCanvasAdvancedNodePortLayout } from "./canvas-renderers.js";
import {
  getViewportControllerService,
  clear,
  createElement,
  clamp,
...
function measureRenderedNodeSizesWrapper(state) {
  return !!runtimeMeasureRenderedNodeSizes(state);
}

function showPopoverLayer(state, anchorElement, annotation) {
  showPopover(state, anchorElement, annotation);
}

function openNodeMetadata(state, node, section, anchorElement) {
  openNodeMetadataMenu(state, node, section, anchorElement);
}

function buildDiagnosticsSnapshotWrapper(state, bounds) {
  return runtimeBuildDiagnosticsSnapshot(state, bounds);
}
...
function resolveAnchorDirection(side, fallbackDirection) {
  ...
  return side === "left" || side === "top" ? "input" : "output";
}
...
function resolveAnchorDetailMode(options) {
  ...
  return typeof options?.detailMode === "string" ? options.detailMode : "";
}
...
export function resolveLinkAnchorSides(state, source, target, detailMode) {
...
function buildPortAnchorPoint(
  ...
  if ((inputPorts.length > 0 || outputPorts.length > 0) && (side === "left" || side === "right")) {
    ...
    const advancedLayout = getCanvasAdvancedNodePortLayout?.(state, node, hostBounds, detailMode);
    let anchorHostPoint = null;
    if (advancedLayout) {
      const entries =
        side === "right" ? advancedLayout.outputEntries || [] : advancedLayout.inputEntries || [];
      const matchingEntry =
        entries.find((entry) => entry?.port?.id === (port?.id || "")) || entries[index] || null;
    ...
    if (!anchorHostPoint) {
      ...
      const outputColumnLeft = hasBothSides ? hostRight - padding - columnWidth : inputColumnLeft;
...
export function getLinkAnchorPoint(state, node, side, portId, direction, options) {
...
function getCollapseAnchorTargets(state, node) {
  ...
  const directChildren = surfaceNodes.filter(
    (candidate) => candidate?.parentId === node.id && candidate.id !== node.id,
  );
...
export function resolveCollapseAnchorInfo(state, node) {
  const position = getNodePosition(state, node);
  const size = getNodeSize(state, node);
  ...
  return {
    side,
    world: {
      x: side === "right" ? position.x + size.width / 2 : position.x - size.width / 2,
      y: position.y,
    },
  };
}

export function getLinkRetainedKey(link, index) {
  if (
    link?.sourceId ||
    link?.targetId ||
    link?.kind ||
...
export function getLinkPathData(state, source, target, link, options) {
  const detailMode = resolveAnchorDetailMode(options);
  const anchorSides = resolveLinkAnchorSides(state, source, target, detailMode);
  const sourceAnchor = getLinkAnchorPoint(
    state,
    source,
  ...
  const targetAnchor = getLinkAnchorPoint(
    state,
    target,
  ...
  const controlOffset = Math.max(
    92,
    Math.max(Math.abs(targetAnchor.x - sourceAnchor.x), Math.abs(targetAnchor.y - sourceAnchor.y)) *
      0.38,
  );
...
export function updateLinkElement(path, link, pathData) {
  path.setAttribute("d", pathData);
  path.setAttribute("fill", "none");
  path.setAttribute(
    "stroke",
    link.isUserAuthored ? "rgba(14, 165, 233, 0.78)" : "rgba(100, 116, 139, 0.4)",
  );
  ...
  if (shouldRenderArrow(link)) {
    path.setAttribute(
      "marker-end",
      link.isUserAuthored ? "url(#cw-link-arrow-user)" : "url(#cw-link-arrow-system)",
    );
  } else {
...
export function shouldRenderArrow(link) {
  if (!link) {
    return false;
  }

  const kind = (link.kind || "").toLowerCase();
  return !!link.isUserAuthored || kind === "dependson" || kind === "derivedfrom" || kind === "uses";
}

export function getExpandedFrameNodeIds(state, frame) {
...
export function getFrameRetainedKey(frame, index) {
  return frame?.id || `frame:${index}`;
}

export function createFrameElement(state, frameId) {
...
export function updateFrameElement(entry, frame, frameId, memberNodes, bounds) {
...
export function getFrameBounds(state, memberNodes) {
...
export function legacyRenderGroupFrames(state, visibleNodes) {
  if (!state.frameLayer) {
    return;
  }
...
export function resolveChipToneClass(tone) {
...
export function createProgressMarker(state, node) {
...
export function resolveProgressDisplay(progressMode, progressPercent) {
...
export function createProgressBadge(
  ...
  const marker = createElement(
    document,
    "span",
    `cw-node__progress is-${display.mode}${extraClassName ? ` ${extraClassName}` : ""}`,
  );
  marker.style.setProperty("--cw-progress-angle", `${display.angle}deg`);
  const centerText =
    typeof centerTextOverride === "string" ? centerTextOverride : display.centerText;
...
export function resolveProgressPresetBadgeOptions(iconKey) {
...
export function resolveMarkerGlyph(markerIcon) {
...
export function resolveNodeMarkers(node) {
  const markers = [];
  const seen = new Set();
...
function createMarkerBadgeElement(state, marker, extraClass) {
  ...
  const badge = createElement(
    state.document,
    "span",
    `cw-node__badge cw-node__marker tone-${tone}${extraClass ? ` ${extraClass}` : ""}`,
    glyph,
  );
...
export function createMarkerBadge(state, node) {
  const markers = resolveNodeMarkers(node);
  return markers.length === 0 ? null : createMarkerBadgeElement(state, markers[0], "");
}

export function createMarkerBadges(state, node, maxVisible) {
  const markers = resolveNodeMarkers(node);
  if (markers.length === 0) {
    return [];
  }
  ...
  if (overflowCount > 0) {
    const overflowBadge = createElement(
      state.document,
      "span",
      "cw-node__badge cw-node__marker tone-primary cw-node__marker-overflow",
      `+${overflowCount}`,
    );
...
export function createPriorityBadge(state, node) {
  ...
  const badge = createElement(
    state.document,
    "span",
    `cw-node__badge cw-node__priority is-level-${priority}`,
    `${priority}`,
  );
...
export function appendNodeIndicators(state, node, container) {
...
export function renderInlineTextNode(state, node, nodeElement) {
  ...
  if (
    node.statusPill ||
    node.progressMode ||
    resolveNodeMarkers(node).length > 0 ||
    node.priority > 0
  ) {
    ...
    if (node.statusPill) {
      meta.appendChild(
        createElement(state.document, "span", "cw-node__chip tone-accent", node.statusPill),
      );
    }
...
export function createNodeMedia(state, node) {
  ...
  const media = createElement(
    state.document,
    `div`,
    `cw-node__media cw-node__media--${node.mediaKind}`,
  );
  if (node.mediaKind === "image") {
    ...
    placeholder.appendChild(
      createElement(state.document, "span", "cw-node__media-video-icon", "\u25B6"),
    );
    placeholder.appendChild(
      createElement(state.document, "span", "cw-node__media-video-label", "Preview"),
    );
  ...
  media.appendChild(
    createElement(
      state.document,
      "span",
      "cw-node__media-badge",
      node.mediaKind === "image" ? "Image" : "Video",
    ),
  );
  return media;
}
...
export function createCompactPathButton(state, node) {
  ...
  const text = createElement(
    ...
    "span",
    "cw-node__path-text",
    compactPath.displayText || compactPath.fullPath,
  );
  const action = createElement(
    state.document,
    "span",
    "cw-node__path-action",
    resolveActionGlyph("copy"),
  );
...
export function renderStandardNode(state, node, nodeElement) {
  const surface = createElement(state.document, "div", "cw-node__surface");
  ...
  const icon = createElement(
    ...
    "span",
    "cw-node__icon",
    node.icon || node.kind || "node",
  );
  const kicker = createElement(
    state.document,
    "span",
    "cw-node__kicker",
    node.kind || node.family || "item",
  );
  ...
  if (node.durationLabel) {
    rightMeta.appendChild(
      createElement(state.document, "span", "cw-node__pill", node.durationLabel),
    );
  }
  ...
  if (node.chips.length > 0) {
    ...
    for (const chip of node.chips) {
      chipRow.appendChild(
        createElement(state.document, "span", resolveChipToneClass(chip.tone), chip.text),
      );
    }
  ...
  if (node.branchLabel) {
    footerLeft.appendChild(
      createElement(state.document, "span", "cw-node__chip", node.branchLabel),
    );
  }
...
function syncNodeCollapseAffordance(state, node, nodeElement) {
  ...
  collapse.setAttribute(
    "aria-label",
    state.collapsedIds.has(node.id) ? "Show child nodes" : "Hide child nodes",
  );
}

export function createRetainedNodeElement(state, nodeId) {
...
export function getNodeRetainedContentKey(node, isCollapsed) {
  if (!node) {
...
export function updateNodeElementChrome(state, node, nodeElement, position) {
...
export function renderNodeElementContent(state, node, nodeElement) {
  clear(nodeElement);
  if (node.isInlineTextNode) {
...
export function buildActiveDragContext(state) {
...
export function positionFloatingOverlayWithinHost(state, element, anchorRect) {
...
export function hidePopover(state) {
  if (!state?.popover) {
    return;
  }
...
export function legacyShowPopover(state, anchorElement, annotation) {
  if (
  ...
  state.popoverBody.textContent =
    annotation.description || annotation.label || "Shared workbench signal";
...
export function invokeAnnotationAction(state, node, annotation) {
...
export function renderNodeAnnotations(state, node, container) {
  ...
  for (const annotation of node.annotations) {
    const badge = createElement(
      state.document,
      "button",
      `cw-node__annotation tone-${annotation.tone || "accent"}`,
    );
    badge.type = "button";
...
export function updateConnectorAnchorHover(state, nodeId) {
...
export function getConnectorAnchorPoints(state, node, placementMode) {
  ...
  if (inputPorts.length > 0 || outputPorts.length > 0) {
    return [
      ...inputPorts.map((port, index) =>
        buildPortAnchorPoint(state, node, port, index, inputPorts.length, "left", "input"),
      ),
      ...outputPorts.map((port, index) =>
        buildPortAnchorPoint(state, node, port, index, outputPorts.length, "right", "output"),
      ),
    ];
  }
...
export function hideStatusNotice(state) {
...
export function showStatusNotice(state, message, tone) {
...
export function renderEmptyStateOverlay(state, visibleNodes) {
  if (!state?.emptyState) {
    return;
  }
  ...
  state.emptyStateBody.textContent =
    state.surface.chrome.emptyStateDescription || "Use quick create to start building the scene.";
}

export function clearSnapGuides(state) {
  state.snapGuides = [];
}

export function legacyRenderSnapGuides(state) {
  if (!state?.guideLayer) {
    return;
  }
  ...
  for (const guide of state.snapGuides) {
    const element = createElement(
      state.document,
      "div",
      `cw-snap-guide is-${guide.orientation || "vertical"}`,
    );
...
export function legacyRenderConnectorAnchorOverlay(state, visibleNodes) {
  ...
  for (const nodeId of activeIds) {
    ...
    for (const point of getConnectorAnchorPoints(state, node, anchors.placementMode)) {
      ...
      if (point.accentColor) {
        const anchorRing = hexToRgba?.(point.accentColor, 0.2) || "rgba(125, 211, 252, 0.16)";
        const anchorShadow = hexToRgba?.(point.accentColor, 0.18) || "rgba(14, 165, 233, 0.18)";
...
export function getSelectionBounds(state, visibleNodes) {
...
export function legacyRenderTransformHandlesOverlay(state, visibleNodes) {
...
export function resolveSnapAdjustment(state, interaction, deltaX, deltaY) {
  ...
  for (const movingNode of movingNodes) {
    ...
    for (const stationaryNode of stationaryNodes) {
      ...
      if (
        ...
        (!bestX || Math.abs(offsetX) < Math.abs(bestX.offset))
      ) {
      ...
      if (
        ...
        (!bestY || Math.abs(offsetY) < Math.abs(bestY.offset))
      ) {
...
export function legacyRenderDebugDecorations(state, visibleNodes) {
  if (!state?.debugLayer) {
    return;
  }
...
export function legacyBuildDiagnosticsSnapshot(state, bounds) {
  return {
...
export function renderDiagnosticsOverlay(state, _visibleNodes) {
  ...
  const snapshot = buildDiagnosticsSnapshotWrapper(state, bounds);
  ...
  const rows = [
    ...
    [
      "Last drag patch",
      `${snapshot.metrics.lastDragPatchedNodeCount}/${snapshot.metrics.lastDragPatchedLinkCount}/${snapshot.metrics.lastDragPatchedFrameCount}`,
    ],
  ...
  if (diagnostics.showViewportStats && bounds) {
    rows.push([
      "Bounds",
      `${round(bounds.minX)}:${round(bounds.minY)} to ${round(bounds.maxX)}:${round(bounds.maxY)}`,
    ]);
  }
...
export function navigateViaMinimap(state, event) {
...
export async function writeClipboardText(payload) {
  if (!payload) {
    return false;
  }
...
export function resolveClipboardAnchor(state) {
  const rect = state.host.getBoundingClientRect();
  return getWorldPoint(state, rect.left + rect.width / 2, rect.top + rect.height / 2);
}

export function buildClipboardPayload(state, action) {
  ...
  return {
    ...
    selectedNodeIds,
    anchorWorld:
      action === "Paste" || action === "Duplicate" ? resolveClipboardAnchor(state) : null,
  };
}
...
export function copySelectionToClipboard(state, shouldDispatch = true) {
  return requestClipboardAction(state, "Copy", shouldDispatch);
}

export function requestClipboardCut(state, shouldDispatch = true) {
  return requestClipboardAction(state, "Cut", shouldDispatch);
}

export function requestClipboardPaste(state, shouldDispatch = true) {
  return requestClipboardAction(state, "Paste", shouldDispatch);
}

export function requestClipboardDuplicate(state, shouldDispatch = true) {
...
export function toggleDiagnostics(state) {
...
export function invalidateMeasuredLayout(state) {
  state.layoutPositions = null;
  state.layoutKey = "";
}

export function legacyMeasureRenderedNodeSizes(state) {
  if (!state.nodeLayer) {
    return false;
  }
...
export function legacyScheduleNodeMeasurement(state) {
  ...
  state.measureLayoutFrame = window.requestAnimationFrame(() => {
    ...
    if (measureRenderedNodeSizesWrapper(state)) {
      renderState(state);
    }
  });
}

export function getHostPoint(state, clientX, clientY) {
  const rect = state.host.getBoundingClientRect();
  return {
    x: clientX - rect.left,
    y: clientY - rect.top,
...
export function worldToHostPoint(state, point) {
...
export function getWorldPoint(state, clientX, clientY) {
