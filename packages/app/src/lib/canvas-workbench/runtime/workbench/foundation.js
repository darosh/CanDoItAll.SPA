import { worldToHostPoint } from "./layout-and-legacy-render.js";
import { applySceneTransform } from "./runtime-rendering.js";
import { render, publishState } from "./interaction-and-state.js";
import * as selectionModel from "../services/selection-model.js";
import * as textMeasureService from "../services/text-measure-service.js";
import * as viewportControllerService from "../services/viewport-controller.js";
import * as animationTimelineService from "../services/animation-timeline.js";
import * as themeTokens from "../theme-tokens.js";

export { selectionModel };

export const contextSubmenuHoverDelayMs = 500;
export const MIN_ZOOM = 0.15;
export const MAX_ZOOM = 1.75;

export function getTextMeasureService() {
  return textMeasureService;
}
export function getViewportControllerService() {
  return viewportControllerService;
}
export function getAnimationTimelineService() {
  return animationTimelineService;
}
...
export function createElement(document, tagName, className, text) {
...
export function createSvgElement(document, tagName, className) {
...
export function normalizeInputField(field) {
  return {
    key: field?.key || "",
...
export function createWorkbenchMetrics() {
  return {
    renderCount: 0,
...
export function formatMetricDuration(value) {
...
export function cloneWorkbenchMetrics(metrics) {
  return {
...
export function incrementMetric(metrics, key) {
...
export function resetLastDragPatchMetrics(metrics) {
  if (!metrics) {
    return;
  }
...
export function recordDragPatchMetrics(metrics, nodeCount, linkCount, frameCount) {
  if (!metrics) {
    return;
  }
...
export function normalizeAction(action) {
  return {
    ...
    inputFields: Array.isArray(action?.inputFields)
      ? action.inputFields.map(normalizeInputField)
      : [],
    defaultInputValues: Array.isArray(action?.defaultInputValues)
      ? action.defaultInputValues.map(normalizeInputValue)
      : [],
...
export function normalizeCompactPath(path) {
...
export function normalizeDiagnosticsOptions(options) {
  return {
...
export function normalizeMinimapOptions(options) {
  return {
...
export function normalizeTooltipPopoverOptions(options) {
  return {
    isEnabled: options?.isEnabled !== false,
...
export function normalizeMarqueeOptions(options) {
...
export function normalizeSnapGuideOptions(options) {
  const tolerance =
    typeof options?.tolerance === "number" && Number.isFinite(options.tolerance)
      ? Math.max(0, options.tolerance)
      : 18;
  const modifierPolicy = (options?.modifierPolicy || "ShiftBypassesSnap")
    .toString()
    .trim()
    .toLowerCase();
  return {
...
export function normalizeConnectorAnchorOptions(options) {
...
export function normalizeTransformHandleOptions(options) {
  const placementMode = (options?.placementMode || "SelectionBounds")
    .toString()
    .trim()
    .toLowerCase();
  return {
...
export function normalizeGroupFrame(frame) {
  return {
    id: frame?.id || "",
...
export function normalizeMenuActionScale(value) {
  if (typeof value !== "number" || Number.isNaN(value)) {
    return 1;
  }
...
export function normalizeSurface(surface) {
  ...
  const normalizedViewport = viewportController?.normalizeUiState?.(surface?.uiState) || {
    zoom:
      typeof surface?.uiState?.zoom === "number"
        ? clamp(surface.uiState.zoom, MIN_ZOOM, MAX_ZOOM)
        : 1,
  ...
  return {
    ...
    nodes: Array.isArray(surface?.nodes)
      ? surface.nodes.map((node) => ({
          ...
          contextActions: Array.isArray(node.contextActions)
            ? node.contextActions.map(normalizeAction)
            : [],
          ...
          annotations: Array.isArray(node?.annotations)
            ? node.annotations.map(normalizeAnnotation)
            : [],
          inputPorts: normalizePortCollection(node?.inputPorts, "left"),
          outputPorts: normalizePortCollection(node?.outputPorts, "right"),
        }))
      : [],
    links: Array.isArray(surface?.links)
    ...
    uiState: {
      ...
      highlightedNodeIds: Array.isArray(surface?.uiState?.highlightedNodeIds)
        ...
        : [],
      collapsedNodeIds: Array.isArray(surface?.uiState?.collapsedNodeIds)
        ? [...surface.uiState.collapsedNodeIds]
        : [],
      groupFrames: Array.isArray(surface?.uiState?.groupFrames)
        ? surface.uiState.groupFrames.map(normalizeGroupFrame)
        : [],
    ...
    chrome: {
      quickCreateActions: Array.isArray(surface?.chrome?.quickCreateActions)
        ? surface.chrome.quickCreateActions.map(normalizeAction)
        : [],
      groupContextActions: Array.isArray(surface?.chrome?.groupContextActions)
        ? surface.chrome.groupContextActions.map(normalizeAction)
        : [],
      ...
      emptyStateDescription:
        surface?.chrome?.emptyStateDescription || "Use quick create to start building the scene.",
...
export function toSelectionSet(selectedNodeIds) {
  return new Set((selectedNodeIds || []).filter(Boolean));
}

export function toCollapsedSet(collapsedNodeIds) {
  return new Set((collapsedNodeIds || []).filter(Boolean));
}

export function getDefaultNodeSize(node) {
...
function countInlineTextNodeMetaItems(node) {
  const markerCount =
    Array.isArray(node?.markers) && node.markers.length > 0
...
function estimateInlineTextNodeSize(node, baseSize, measureService) {
  ...
  const estimatedHeight = clamp(Math.ceil(30 + textMeasure.estimatedHeight + metaHeight), 76, 304);
...
export function resolveBaseNodeSize(node) {
  if (node.isInlineTextNode) {
    return { width: 164, height: 76 };
  }

  if (
    (node?.family || "").toLowerCase() === "workflow-decision" ||
...
export function estimateNodeSizeFromText(node, baseSize) {
  ...
  const inputPorts = Array.isArray(node?.inputPorts) ? node.inputPorts : [];
  const outputPorts = Array.isArray(node?.outputPorts) ? node.outputPorts : [];
  const longestInputPort = inputPorts.reduce(
    (longest, port) => Math.max(longest, (port?.label || port?.id || "").length),
    0,
  );
  const longestOutputPort = outputPorts.reduce(
    (longest, port) => Math.max(longest, (port?.label || port?.id || "").length),
    0,
  );
  const portRows = Math.max(inputPorts.length, outputPorts.length);
  const portWidthBudget =
    portRows > 0
      ? Math.min(420, Math.max(baseSize.width, 224 + (longestInputPort + longestOutputPort) * 5.8))
      : 0;
  const portHeight = portRows > 0 ? Math.max(52, 28 + portRows * 26) : 0;
  const annotationHeight = Array.isArray(node.annotations)
    ? Math.ceil(node.annotations.length / 2) * 14
    : 0;
  const footerHeight =
    (Array.isArray(node.chips) && node.chips.length > 0 ? 18 : 0) +
...
export function getNodeSize(state, node) {
...
export function isNodeVisible(state, nodeId) {
...
export function getProjectionOverscanPx(state, hostRect) {
  ...
  return state?.interaction ? Math.max(baseOverscan, 320) : baseOverscan;
}

export function collectProjectedContextNodeIds(state) {
...
export function isNodeProjectedInViewport(
  state,
  node,
  visibleNodes,
  overscanPx,
  ...
  const projectPoint = worldToHostPoint;
  ...
  return (
    position.x + halfWidth >= -overscanPx &&
...
export function getProjectedNodes(state, visibleNodes) {
  ...
  const projectedNodes = visibleNodes.filter(
    (node) =>
...
export function getBaseNodePosition(state, node) {
...
export function getNodeDepth(state, nodeId, cache) {
  if (cache.has(nodeId)) {
...
export function getNodeMobility(state, node) {
  if (Array.isArray(state.interaction?.nodeIds) && state.interaction.nodeIds.includes(node.id)) {
    return 1;
  }
...
export function buildResolvedLayoutKey(state, visibleNodes) {
  return visibleNodes
    .map((node) => {
      const base = getBaseNodePosition(state, node);
      const size = getNodeSize(state, node);
      const isActiveDragNode =
        Array.isArray(state.interaction?.nodeIds) && state.interaction.nodeIds.includes(node.id);
      return [
        node.id,
        node.parentId || "",
        round(base.x),
...
export function buildLayoutItems(state, visibleNodes) {
...
export function getCollisionPaddingX(first, second) {
  let padding = 28;
  ...
  if (
    (first.node.family || "").toLowerCase() === "root" ||
    (second.node.family || "").toLowerCase() === "root"
  ) {
    padding += 18;
  }
...
export function getCollisionPaddingY(first, second) {
  let padding = 24;
...
export function getOverlapDelta(first, second, firstPosition, secondPosition) {
  ...
  const overlapX =
    ...
    getCollisionPaddingX(first, second) -
    Math.abs(deltaX);
  const overlapY =
    (first.size.height + second.size.height) / 2 +
    getCollisionPaddingY(first, second) -
    Math.abs(deltaY);
  return { deltaX, deltaY, overlapX, overlapY };
}

export function chooseCollisionAxis(first, second, overlap) {
  if (first.parentId === second.id || second.parentId === first.id) {
    return "x";
  }
...
export function resolveCollisionDirection(first, second, axis, overlap) {
...
export function applyCollisionSeparation(
  first,
  second,
  firstPosition,
  secondPosition,
...
export function enforceParentClearance(itemsById, positions) {
  let moved = false;
...
export function enforceSiblingSpacing(itemsById, positions) {
  ...
  for (const siblings of groups.values()) {
    ...
    const desiredCenter =
      siblings.reduce((total, item) => total + item.base.y, 0) / Math.max(1, siblings.length);
    const actualCenter =
      siblings.reduce((total, item) => total + positions.get(item.id).y, 0) /
...
export function relaxTowardBase(items, positions) {
  let moved = false;
...
export function computeResolvedNodePositions(state, visibleNodes) {
...
export function ensureLayoutPositions(state, visibleNodes) {
...
export function getNodePosition(state, node, visibleNodes) {
  const resolved = ensureLayoutPositions(state, visibleNodes).get(node.id);
  return resolved ? { x: resolved.x, y: resolved.y } : getBaseNodePosition(state, node);
}

export function getSceneBounds(state, visibleNodes) {
  const nodes = Array.isArray(visibleNodes) ? visibleNodes : getVisibleNodes(state);
  if (!nodes.length) {
    return null;
  }
  ...
  for (const node of nodes) {
    ...
    const position = resolved ? { x: resolved.x, y: resolved.y } : getBaseNodePosition(state, node);
    const size = getNodeSize(state, node);
...
export function clampPanToScene(state, panX, panY, zoom) {
...
export function setPan(state, panX, panY, zoom, options) {
...
export function syncMenuScaleCss(state) {
  if (!state?.host) {
    return;
  }
  ...
  state.host.style.setProperty(
    "--cw-menu-scale",
    `${normalizeMenuActionScale(state.ui?.menuActionScale)}`,
  );
}
...
export function serializeState(state) {
  return JSON.stringify({
    ...
    groupFrames: Array.isArray(state.ui.groupFrames)
      ? state.ui.groupFrames.map(normalizeGroupFrame)
      : [],
...
export function legacyApplySceneTransform(state) {
  state.scene.style.transform = `translate(${state.ui.panX}px, ${state.ui.panY}px) scale(${state.ui.zoom})`;
}

export function cancelViewportAnimation(state) {
...
export function updateViewportTransform(state, viewport, options) {
  state.ui.zoom = viewport.zoom;
  setPan(state, viewport.panX, viewport.panY, viewport.zoom, options);
  applySceneTransformNow(state);
}

export function animateViewportTransition(state, target, options) {
  if (!target) {
    return false;
  }
  ...
  const hasMeaningfulChange =
    Math.abs(current.panX - target.panX) > 0.5 ||
...
export const canvasColorTokenMap = {
  ...
  nodeReadonlySurfaceFill: {
    cssVar: "--ui-canvas-node-readonly-surface-fill",
    fallback: "rgba(248, 250, 252, 0.98)",
  },
  nodeReadonlySurfaceStrokeSelected: {
    cssVar: "--ui-canvas-node-readonly-surface-stroke-selected",
    fallback: "rgba(71, 85, 105, 0.9)",
  },
  nodeReadonlySurfaceStrokeUnselected: {
    cssVar: "--ui-canvas-node-readonly-surface-stroke",
    fallback: "rgba(148, 163, 184, 0.34)",
  },
  nodeReadonlySurfaceShadow: {
    cssVar: "--ui-canvas-node-readonly-surface-shadow",
    fallback: "rgba(100, 116, 139, 0.12)",
  },
  nodeReadonlyLabelText: {
    cssVar: "--ui-canvas-node-readonly-label-text",
    fallback: "rgba(71, 85, 105, 0.76)",
  },
  nodeReadonlyTitleText: {
    cssVar: "--ui-canvas-node-readonly-title-text",
    fallback: "rgba(51, 65, 85, 0.94)",
  },
  nodeReadonlySecondaryText: {
    cssVar: "--ui-canvas-node-readonly-secondary-text",
    fallback: "rgba(100, 116, 139, 0.92)",
  },
  nodeReadonlyIconFill: {
    cssVar: "--ui-canvas-node-readonly-icon-fill",
    fallback: "rgba(241, 245, 249, 0.94)",
  },
  nodeReadonlyIconStroke: {
    cssVar: "--ui-canvas-node-readonly-icon-stroke",
    fallback: "rgba(148, 163, 184, 0.28)",
  },
  nodeReadonlyIconText: {
    cssVar: "--ui-canvas-node-readonly-icon-text",
    fallback: "rgba(71, 85, 105, 0.96)",
  },
  nodeReadonlySubtleFill: {
    cssVar: "--ui-canvas-node-readonly-subtle-fill",
    fallback: "rgba(255, 255, 255, 0.86)",
  },
  nodeReadonlySubtleStroke: {
    cssVar: "--ui-canvas-node-readonly-subtle-stroke",
    fallback: "rgba(148, 163, 184, 0.3)",
  },
  nodeReadonlySubtleText: {
    cssVar: "--ui-canvas-node-readonly-subtle-text",
    fallback: "rgba(51, 65, 85, 0.9)",
  },
  nodeReadonlyProgressTrack: {
    cssVar: "--ui-canvas-node-readonly-progress-track",
    fallback: "rgba(148, 163, 184, 0.28)",
  },
  nodeReadonlyProgressText: {
    cssVar: "--ui-canvas-node-readonly-progress-text",
    fallback: "rgba(71, 85, 105, 0.9)",
  },
  nodePrimarySurfaceFill: {
    cssVar: "--ui-canvas-node-primary-surface-fill",
    fallback: "rgba(15, 23, 42, 0.98)",
  },
  nodePrimarySurfaceStrokeSelected: {
    cssVar: "--ui-canvas-node-primary-surface-stroke-selected",
    fallback: "rgba(248, 250, 252, 0.94)",
  },
  nodePrimarySurfaceStrokeUnselected: {
    cssVar: "--ui-canvas-node-primary-surface-stroke",
    fallback: "rgba(51, 65, 85, 0.96)",
  },
  nodePrimarySurfaceShadow: {
    cssVar: "--ui-canvas-node-primary-surface-shadow",
    fallback: "rgba(15, 23, 42, 0.28)",
  },
  nodePrimaryLabelText: {
    cssVar: "--ui-canvas-node-primary-label-text",
    fallback: "rgba(191, 219, 254, 0.78)",
  },
  nodePrimaryTitleText: {
    cssVar: "--ui-canvas-node-primary-title-text",
    fallback: "rgba(248, 250, 252, 0.98)",
  },
  nodePrimarySecondaryText: {
    cssVar: "--ui-canvas-node-primary-secondary-text",
    fallback: "rgba(226, 232, 240, 0.84)",
  },
  nodePrimaryIconFill: {
    cssVar: "--ui-canvas-node-primary-icon-fill",
    fallback: "rgba(255, 255, 255, 0.14)",
  },
  nodePrimaryIconStroke: {
    cssVar: "--ui-canvas-node-primary-icon-stroke",
    fallback: "rgba(255, 255, 255, 0.18)",
  },
  nodePrimaryIconText: {
    cssVar: "--ui-canvas-node-primary-icon-text",
    fallback: "rgba(248, 250, 252, 0.96)",
  },
  nodePrimarySubtleFill: {
    cssVar: "--ui-canvas-node-primary-subtle-fill",
    fallback: "rgba(255, 255, 255, 0.14)",
  },
  nodePrimarySubtleStroke: {
    cssVar: "--ui-canvas-node-primary-subtle-stroke",
    fallback: "rgba(255, 255, 255, 0.18)",
  },
  nodePrimarySubtleText: {
    cssVar: "--ui-canvas-node-primary-subtle-text",
    fallback: "rgba(248, 250, 252, 0.94)",
  },
  nodePrimaryProgressTrack: {
    cssVar: "--ui-canvas-node-primary-progress-track",
    fallback: "rgba(248, 250, 252, 0.24)",
  },
  nodePrimaryProgressText: {
    cssVar: "--ui-canvas-node-primary-progress-text",
    fallback: "rgba(248, 250, 252, 0.92)",
  },
  nodeSecondarySurfaceFill: {
    cssVar: "--ui-canvas-node-secondary-surface-fill",
    fallback: "rgba(237, 233, 254, 0.98)",
  },
  nodeSecondarySurfaceStrokeSelected: {
    cssVar: "--ui-canvas-node-secondary-surface-stroke-selected",
    fallback: "rgba(124, 58, 237, 0.92)",
  },
  nodeSecondarySurfaceStrokeUnselected: {
    cssVar: "--ui-canvas-node-secondary-surface-stroke",
    fallback: "rgba(167, 139, 250, 0.62)",
  },
  nodeSecondarySurfaceShadow: {
    cssVar: "--ui-canvas-node-secondary-surface-shadow",
    fallback: "rgba(109, 40, 217, 0.16)",
  },
  nodeSecondaryLabelText: {
    cssVar: "--ui-canvas-node-secondary-label-text",
    fallback: "rgba(109, 40, 217, 0.72)",
  },
  nodeSecondaryTitleText: {
    cssVar: "--ui-canvas-node-secondary-title-text",
    fallback: "rgba(88, 28, 135, 0.94)",
  },
  nodeSecondarySecondaryText: {
    cssVar: "--ui-canvas-node-secondary-secondary-text",
    fallback: "rgba(107, 33, 168, 0.82)",
  },
  nodeSecondaryIconFill: {
    cssVar: "--ui-canvas-node-secondary-icon-fill",
    fallback: "rgba(255, 255, 255, 0.64)",
  },
  nodeSecondaryIconStroke: {
    cssVar: "--ui-canvas-node-secondary-icon-stroke",
    fallback: "rgba(167, 139, 250, 0.4)",
  },
  nodeSecondaryIconText: {
    cssVar: "--ui-canvas-node-secondary-icon-text",
    fallback: "rgba(109, 40, 217, 0.94)",
  },
  nodeSecondarySubtleFill: {
    cssVar: "--ui-canvas-node-secondary-subtle-fill",
    fallback: "rgba(255, 255, 255, 0.74)",
  },
  nodeSecondarySubtleStroke: {
    cssVar: "--ui-canvas-node-secondary-subtle-stroke",
    fallback: "rgba(196, 181, 253, 0.44)",
  },
  nodeSecondarySubtleText: {
    cssVar: "--ui-canvas-node-secondary-subtle-text",
    fallback: "rgba(88, 28, 135, 0.88)",
  },
  nodeSecondaryProgressTrack: {
    cssVar: "--ui-canvas-node-secondary-progress-track",
    fallback: "rgba(139, 92, 246, 0.26)",
  },
  nodeSecondaryProgressText: {
    cssVar: "--ui-canvas-node-secondary-progress-text",
    fallback: "rgba(88, 28, 135, 0.88)",
  },
  nodeSuccessSurfaceFill: {
    cssVar: "--ui-canvas-node-success-surface-fill",
    fallback: "rgba(220, 252, 231, 0.98)",
  },
  nodeSuccessSurfaceStrokeSelected: {
    cssVar: "--ui-canvas-node-success-surface-stroke-selected",
    fallback: "rgba(22, 163, 74, 0.92)",
  },
  nodeSuccessSurfaceStrokeUnselected: {
    cssVar: "--ui-canvas-node-success-surface-stroke",
    fallback: "rgba(74, 222, 128, 0.62)",
  },
  nodeSuccessSurfaceShadow: {
    cssVar: "--ui-canvas-node-success-surface-shadow",
    fallback: "rgba(22, 163, 74, 0.14)",
  },
  nodeSuccessLabelText: {
    cssVar: "--ui-canvas-node-success-label-text",
    fallback: "rgba(21, 128, 61, 0.72)",
  },
  nodeSuccessTitleText: {
    cssVar: "--ui-canvas-node-success-title-text",
    fallback: "rgba(20, 83, 45, 0.95)",
  },
  nodeSuccessSecondaryText: {
    cssVar: "--ui-canvas-node-success-secondary-text",
    fallback: "rgba(21, 128, 61, 0.82)",
  },
  nodeSuccessIconFill: {
    cssVar: "--ui-canvas-node-success-icon-fill",
    fallback: "rgba(255, 255, 255, 0.62)",
  },
  nodeSuccessIconStroke: {
    cssVar: "--ui-canvas-node-success-icon-stroke",
    fallback: "rgba(74, 222, 128, 0.42)",
  },
  nodeSuccessIconText: {
    cssVar: "--ui-canvas-node-success-icon-text",
    fallback: "rgba(21, 128, 61, 0.94)",
  },
  nodeSuccessSubtleFill: {
    cssVar: "--ui-canvas-node-success-subtle-fill",
    fallback: "rgba(255, 255, 255, 0.76)",
  },
  nodeSuccessSubtleStroke: {
    cssVar: "--ui-canvas-node-success-subtle-stroke",
    fallback: "rgba(134, 239, 172, 0.46)",
  },
  nodeSuccessSubtleText: {
    cssVar: "--ui-canvas-node-success-subtle-text",
    fallback: "rgba(20, 83, 45, 0.88)",
  },
  nodeSuccessProgressTrack: {
    cssVar: "--ui-canvas-node-success-progress-track",
    fallback: "rgba(22, 163, 74, 0.22)",
  },
  nodeSuccessProgressText: {
    cssVar: "--ui-canvas-node-success-progress-text",
    fallback: "rgba(20, 83, 45, 0.88)",
  },
  nodeInfoSurfaceFill: {
    cssVar: "--ui-canvas-node-info-surface-fill",
    fallback: "rgba(224, 242, 254, 0.98)",
  },
  nodeInfoSurfaceStrokeSelected: {
    cssVar: "--ui-canvas-node-info-surface-stroke-selected",
    fallback: "rgba(2, 132, 199, 0.92)",
  },
  nodeInfoSurfaceStrokeUnselected: {
    cssVar: "--ui-canvas-node-info-surface-stroke",
    fallback: "rgba(125, 211, 252, 0.64)",
  },
  nodeInfoSurfaceShadow: {
    cssVar: "--ui-canvas-node-info-surface-shadow",
    fallback: "rgba(2, 132, 199, 0.15)",
  },
  nodeInfoLabelText: {
    cssVar: "--ui-canvas-node-info-label-text",
    fallback: "rgba(14, 116, 144, 0.72)",
  },
  nodeInfoTitleText: {
    cssVar: "--ui-canvas-node-info-title-text",
    fallback: "rgba(12, 74, 110, 0.95)",
  },
  nodeInfoSecondaryText: {
    cssVar: "--ui-canvas-node-info-secondary-text",
    fallback: "rgba(14, 116, 144, 0.82)",
  },
  nodeInfoIconFill: {
    cssVar: "--ui-canvas-node-info-icon-fill",
    fallback: "rgba(255, 255, 255, 0.62)",
  },
  nodeInfoIconStroke: {
    cssVar: "--ui-canvas-node-info-icon-stroke",
    fallback: "rgba(125, 211, 252, 0.44)",
  },
  nodeInfoIconText: {
    cssVar: "--ui-canvas-node-info-icon-text",
    fallback: "rgba(2, 132, 199, 0.94)",
  },
  nodeInfoSubtleFill: {
    cssVar: "--ui-canvas-node-info-subtle-fill",
    fallback: "rgba(255, 255, 255, 0.76)",
  },
  nodeInfoSubtleStroke: {
    cssVar: "--ui-canvas-node-info-subtle-stroke",
    fallback: "rgba(125, 211, 252, 0.46)",
  },
  nodeInfoSubtleText: {
    cssVar: "--ui-canvas-node-info-subtle-text",
    fallback: "rgba(12, 74, 110, 0.88)",
  },
  nodeInfoProgressTrack: {
    cssVar: "--ui-canvas-node-info-progress-track",
    fallback: "rgba(2, 132, 199, 0.22)",
  },
  nodeInfoProgressText: {
    cssVar: "--ui-canvas-node-info-progress-text",
    fallback: "rgba(12, 74, 110, 0.88)",
  },
  nodeWarningSurfaceFill: {
    cssVar: "--ui-canvas-node-warning-surface-fill",
    fallback: "rgba(254, 243, 199, 0.98)",
  },
  nodeWarningSurfaceStrokeSelected: {
    cssVar: "--ui-canvas-node-warning-surface-stroke-selected",
    fallback: "rgba(217, 119, 6, 0.92)",
  },
  nodeWarningSurfaceStrokeUnselected: {
    cssVar: "--ui-canvas-node-warning-surface-stroke",
    fallback: "rgba(251, 191, 36, 0.64)",
  },
  nodeWarningSurfaceShadow: {
    cssVar: "--ui-canvas-node-warning-surface-shadow",
    fallback: "rgba(217, 119, 6, 0.15)",
  },
  nodeWarningLabelText: {
    cssVar: "--ui-canvas-node-warning-label-text",
    fallback: "rgba(180, 83, 9, 0.72)",
  },
  nodeWarningTitleText: {
    cssVar: "--ui-canvas-node-warning-title-text",
    fallback: "rgba(120, 53, 15, 0.95)",
  },
  nodeWarningSecondaryText: {
    cssVar: "--ui-canvas-node-warning-secondary-text",
    fallback: "rgba(146, 64, 14, 0.82)",
  },
  nodeWarningIconFill: {
    cssVar: "--ui-canvas-node-warning-icon-fill",
    fallback: "rgba(255, 255, 255, 0.58)",
  },
  nodeWarningIconStroke: {
    cssVar: "--ui-canvas-node-warning-icon-stroke",
    fallback: "rgba(251, 191, 36, 0.42)",
  },
  nodeWarningIconText: {
    cssVar: "--ui-canvas-node-warning-icon-text",
    fallback: "rgba(180, 83, 9, 0.94)",
  },
  nodeWarningSubtleFill: {
    cssVar: "--ui-canvas-node-warning-subtle-fill",
    fallback: "rgba(255, 255, 255, 0.76)",
  },
  nodeWarningSubtleStroke: {
    cssVar: "--ui-canvas-node-warning-subtle-stroke",
    fallback: "rgba(252, 211, 77, 0.48)",
  },
  nodeWarningSubtleText: {
    cssVar: "--ui-canvas-node-warning-subtle-text",
    fallback: "rgba(120, 53, 15, 0.88)",
  },
  nodeWarningProgressTrack: {
    cssVar: "--ui-canvas-node-warning-progress-track",
    fallback: "rgba(217, 119, 6, 0.22)",
  },
  nodeWarningProgressText: {
    cssVar: "--ui-canvas-node-warning-progress-text",
    fallback: "rgba(120, 53, 15, 0.88)",
  },
  nodeDangerSurfaceFill: {
    cssVar: "--ui-canvas-node-danger-surface-fill",
    fallback: "rgba(254, 226, 226, 0.98)",
  },
  nodeDangerSurfaceStrokeSelected: {
    cssVar: "--ui-canvas-node-danger-surface-stroke-selected",
    fallback: "rgba(220, 38, 38, 0.94)",
  },
  nodeDangerSurfaceStrokeUnselected: {
    cssVar: "--ui-canvas-node-danger-surface-stroke",
    fallback: "rgba(252, 165, 165, 0.7)",
  },
  nodeDangerSurfaceShadow: {
    cssVar: "--ui-canvas-node-danger-surface-shadow",
    fallback: "rgba(220, 38, 38, 0.16)",
  },
  nodeDangerLabelText: {
    cssVar: "--ui-canvas-node-danger-label-text",
    fallback: "rgba(185, 28, 28, 0.72)",
  },
  nodeDangerTitleText: {
    cssVar: "--ui-canvas-node-danger-title-text",
    fallback: "rgba(127, 29, 29, 0.95)",
  },
  nodeDangerSecondaryText: {
    cssVar: "--ui-canvas-node-danger-secondary-text",
    fallback: "rgba(153, 27, 27, 0.82)",
  },
  nodeDangerIconFill: {
    cssVar: "--ui-canvas-node-danger-icon-fill",
    fallback: "rgba(255, 255, 255, 0.6)",
  },
  nodeDangerIconStroke: {
    cssVar: "--ui-canvas-node-danger-icon-stroke",
    fallback: "rgba(252, 165, 165, 0.46)",
  },
  nodeDangerIconText: {
    cssVar: "--ui-canvas-node-danger-icon-text",
    fallback: "rgba(185, 28, 28, 0.95)",
  },
  nodeDangerSubtleFill: {
    cssVar: "--ui-canvas-node-danger-subtle-fill",
    fallback: "rgba(255, 255, 255, 0.8)",
  },
  nodeDangerSubtleStroke: {
    cssVar: "--ui-canvas-node-danger-subtle-stroke",
    fallback: "rgba(252, 165, 165, 0.48)",
  },
  nodeDangerSubtleText: {
    cssVar: "--ui-canvas-node-danger-subtle-text",
    fallback: "rgba(127, 29, 29, 0.88)",
  },
  nodeDangerProgressTrack: {
    cssVar: "--ui-canvas-node-danger-progress-track",
    fallback: "rgba(220, 38, 38, 0.22)",
  },
  nodeDangerProgressText: {
    cssVar: "--ui-canvas-node-danger-progress-text",
    fallback: "rgba(127, 29, 29, 0.88)",
  },
  nodeWorkflowDecisionSurfaceFill: {
    cssVar: "--ui-canvas-node-workflow-decision-surface-fill",
    fallback: "rgba(240, 253, 250, 0.98)",
  },
  nodeWorkflowDecisionSurfaceStrokeSelected: {
    cssVar: "--ui-canvas-node-workflow-decision-surface-stroke-selected",
    fallback: "rgba(15, 118, 110, 0.96)",
  },
  nodeWorkflowDecisionSurfaceStrokeUnselected: {
    cssVar: "--ui-canvas-node-workflow-decision-surface-stroke",
    fallback: "rgba(20, 184, 166, 0.64)",
  },
  nodeWorkflowDecisionSurfaceShadow: {
    cssVar: "--ui-canvas-node-workflow-decision-surface-shadow",
    fallback: "rgba(15, 118, 110, 0.16)",
  },
  nodeWorkflowDecisionLabelText: {
    cssVar: "--ui-canvas-node-workflow-decision-label-text",
    fallback: "rgba(15, 118, 110, 0.78)",
  },
  nodeWorkflowDecisionTitleText: {
    cssVar: "--ui-canvas-node-workflow-decision-title-text",
    fallback: "rgba(19, 78, 74, 0.96)",
  },
  nodeWorkflowDecisionSecondaryText: {
    cssVar: "--ui-canvas-node-workflow-decision-secondary-text",
    fallback: "rgba(15, 118, 110, 0.84)",
  },
  nodeWorkflowDecisionIconFill: {
    cssVar: "--ui-canvas-node-workflow-decision-icon-fill",
    fallback: "rgba(255, 251, 235, 0.86)",
  },
  nodeWorkflowDecisionIconStroke: {
    cssVar: "--ui-canvas-node-workflow-decision-icon-stroke",
    fallback: "rgba(245, 158, 11, 0.46)",
  },
  nodeWorkflowDecisionIconText: {
    cssVar: "--ui-canvas-node-workflow-decision-icon-text",
    fallback: "rgba(15, 118, 110, 0.96)",
  },
  nodeWorkflowDecisionSubtleFill: {
    cssVar: "--ui-canvas-node-workflow-decision-subtle-fill",
    fallback: "rgba(255, 255, 255, 0.84)",
  },
  nodeWorkflowDecisionSubtleStroke: {
    cssVar: "--ui-canvas-node-workflow-decision-subtle-stroke",
    fallback: "rgba(20, 184, 166, 0.4)",
  },
  nodeWorkflowDecisionSubtleText: {
    cssVar: "--ui-canvas-node-workflow-decision-subtle-text",
    fallback: "rgba(15, 118, 110, 0.9)",
  },
  nodeWorkflowDecisionProgressTrack: {
    cssVar: "--ui-canvas-node-workflow-decision-progress-track",
    fallback: "rgba(20, 184, 166, 0.24)",
  },
  nodeWorkflowDecisionProgressText: {
    cssVar: "--ui-canvas-node-workflow-decision-progress-text",
    fallback: "rgba(19, 78, 74, 0.9)",
  },
  nodeNeutralSurfaceFill: {
    cssVar: "--ui-canvas-node-neutral-surface-fill",
    fallback: "rgba(241, 245, 249, 0.98)",
  },
  nodeNeutralSurfaceStrokeSelected: {
    cssVar: "--ui-canvas-node-neutral-surface-stroke-selected",
    fallback: "rgba(71, 85, 105, 0.92)",
  },
  nodeNeutralSurfaceStrokeUnselected: {
    cssVar: "--ui-canvas-node-neutral-surface-stroke",
    fallback: "rgba(148, 163, 184, 0.44)",
  },
  nodeNeutralSurfaceShadow: {
    cssVar: "--ui-canvas-node-neutral-surface-shadow",
    fallback: "rgba(71, 85, 105, 0.1)",
  },
  nodeNeutralLabelText: {
    cssVar: "--ui-canvas-node-neutral-label-text",
    fallback: "rgba(71, 85, 105, 0.7)",
  },
  nodeNeutralTitleText: {
    cssVar: "--ui-canvas-node-neutral-title-text",
    fallback: "rgba(15, 23, 42, 0.94)",
  },
  nodeNeutralSecondaryText: {
    cssVar: "--ui-canvas-node-neutral-secondary-text",
    fallback: "rgba(71, 85, 105, 0.84)",
  },
  nodeNeutralIconFill: {
    cssVar: "--ui-canvas-node-neutral-icon-fill",
    fallback: "rgba(255, 255, 255, 0.72)",
  },
  nodeNeutralIconStroke: {
    cssVar: "--ui-canvas-node-neutral-icon-stroke",
    fallback: "rgba(148, 163, 184, 0.34)",
  },
  nodeNeutralIconText: {
    cssVar: "--ui-canvas-node-neutral-icon-text",
    fallback: "rgba(71, 85, 105, 0.94)",
  },
  nodeNeutralSubtleFill: {
    cssVar: "--ui-canvas-node-neutral-subtle-fill",
    fallback: "rgba(255, 255, 255, 0.82)",
  },
  nodeNeutralSubtleStroke: {
    cssVar: "--ui-canvas-node-neutral-subtle-stroke",
    fallback: "rgba(148, 163, 184, 0.38)",
  },
  nodeNeutralSubtleText: {
    cssVar: "--ui-canvas-node-neutral-subtle-text",
    fallback: "rgba(30, 41, 59, 0.88)",
  },
  nodeNeutralProgressTrack: {
    cssVar: "--ui-canvas-node-neutral-progress-track",
    fallback: "rgba(148, 163, 184, 0.28)",
  },
  nodeNeutralProgressText: {
    cssVar: "--ui-canvas-node-neutral-progress-text",
    fallback: "rgba(51, 65, 85, 0.88)",
  },
  annotationBadgeFill: {
    cssVar: "--ui-canvas-annotation-badge-fill",
    fallback: "rgba(255, 255, 255, 0.94)",
  },
  annotationBadgeStroke: {
    cssVar: "--ui-canvas-annotation-badge-stroke",
    fallback: "rgba(148, 163, 184, 0.24)",
  },
  annotationBadgeText: {
    cssVar: "--ui-canvas-annotation-badge-text",
    fallback: "rgba(15, 23, 42, 0.78)",
  },
  decisionAnchorStrokeDefault: {
    cssVar: "--ui-canvas-decision-anchor-stroke-default",
    fallback: "rgba(20, 184, 166, 0.92)",
  },
  decisionBodyFill: {
    cssVar: "--ui-canvas-decision-body-fill",
    fallback: "rgba(255, 255, 255, 0.98)",
  },
  decisionBranchPillFill: {
    cssVar: "--ui-canvas-decision-branch-pill-fill",
    fallback: "rgba(240, 253, 250, 0.98)",
  },
  decisionBranchPillStroke: {
    cssVar: "--ui-canvas-decision-branch-pill-stroke",
    fallback: "rgba(20, 184, 166, 0.38)",
  },
  decisionBranchPillText: {
    cssVar: "--ui-canvas-decision-branch-pill-text",
    fallback: "rgba(15, 118, 110, 0.98)",
  },
  decisionCueForkDotFill: {
    cssVar: "--ui-canvas-decision-cue-fork-dot-fill",
    fallback: "rgba(15, 118, 110, 0.78)",
  },
  decisionCueForkStroke: {
    cssVar: "--ui-canvas-decision-cue-fork-stroke",
    fallback: "rgba(245, 158, 11, 0.58)",
  },
  decisionCueIconFillDefault: {
    cssVar: "--ui-canvas-decision-cue-icon-fill-default",
    fallback: "rgba(255, 251, 235, 0.9)",
  },
  decisionCueIconStrokeDefault: {
    cssVar: "--ui-canvas-decision-cue-icon-stroke-default",
    fallback: "rgba(245, 158, 11, 0.56)",
  },
  decisionCueShadow: {
    cssVar: "--ui-canvas-decision-cue-shadow",
    fallback: "rgba(15, 118, 110, 0.16)",
  },
  decisionGlyphStroke: {
    cssVar: "--ui-canvas-decision-glyph-stroke",
    fallback: "rgba(51, 65, 85, 0.9)",
  },
  decisionShadowDefault: {
    cssVar: "--ui-canvas-decision-shadow-default",
    fallback: "rgba(15, 23, 42, 0.12)",
  },
  decisionShadowSelected: {
    cssVar: "--ui-canvas-decision-shadow-selected",
    fallback: "rgba(15, 118, 110, 0.2)",
  },
  decisionStrokeDefault: {
    cssVar: "--ui-canvas-decision-stroke-default",
    fallback: "rgba(20, 184, 166, 0.9)",
  },
  decisionTitleText: {
    cssVar: "--ui-canvas-decision-title-text",
    fallback: "rgba(15, 23, 42, 0.92)",
  },
  frameAccentFill: {
    cssVar: "--ui-canvas-frame-accent-fill",
    fallback: "rgba(124, 58, 237, 0.08)",
  },
  frameAccentStroke: {
    cssVar: "--ui-canvas-frame-accent-stroke",
    fallback: "rgba(124, 58, 237, 0.72)",
  },
  frameChipBg: { cssVar: "--ui-canvas-frame-chip-bg", fallback: "rgba(255, 255, 255, 0.96)" },
  frameDangerFill: {
    cssVar: "--ui-canvas-frame-danger-fill",
    fallback: "rgba(220, 38, 38, 0.08)",
  },
  frameDangerStroke: {
    cssVar: "--ui-canvas-frame-danger-stroke",
    fallback: "rgba(220, 38, 38, 0.72)",
  },
  frameLabelText: { cssVar: "--ui-canvas-frame-label-text", fallback: "rgba(15, 23, 42, 0.74)" },
  frameMemberCountText: {
    cssVar: "--ui-canvas-frame-member-count-text",
    fallback: "rgba(71, 85, 105, 0.84)",
  },
  frameSuccessFill: {
    cssVar: "--ui-canvas-frame-success-fill",
    fallback: "rgba(5, 150, 105, 0.08)",
  },
  frameSuccessStroke: {
    cssVar: "--ui-canvas-frame-success-stroke",
    fallback: "rgba(5, 150, 105, 0.72)",
  },
  frameWarningFill: {
    cssVar: "--ui-canvas-frame-warning-fill",
    fallback: "rgba(217, 119, 6, 0.08)",
  },
  frameWarningStroke: {
    cssVar: "--ui-canvas-frame-warning-stroke",
    fallback: "rgba(217, 119, 6, 0.72)",
  },
  linkDangerArrowFill: {
    cssVar: "--ui-canvas-link-danger-arrow-fill",
    fallback: "rgba(225, 29, 72, 0.96)",
  },
  linkDangerStroke: {
    cssVar: "--ui-canvas-link-danger-stroke",
    fallback: "rgba(244, 63, 94, 0.9)",
  },
  linkDefaultArrowFill: {
    cssVar: "--ui-canvas-link-default-arrow-fill",
    fallback: "rgba(100, 116, 139, 0.58)",
  },
  linkDefaultStroke: {
    cssVar: "--ui-canvas-link-default-stroke",
    fallback: "rgba(100, 116, 139, 0.44)",
  },
  linkDependencyArrowFill: {
    cssVar: "--ui-canvas-link-dependency-arrow-fill",
    fallback: "rgba(29, 78, 216, 0.98)",
  },
  linkDependencyStroke: {
    cssVar: "--ui-canvas-link-dependency-stroke",
    fallback: "rgba(37, 99, 235, 0.94)",
  },
  linkHoveredArrowFill: {
    cssVar: "--ui-canvas-link-hovered-arrow-fill",
    fallback: "rgba(220, 38, 38, 0.96)",
  },
  linkHoveredStroke: {
    cssVar: "--ui-canvas-link-hovered-stroke",
    fallback: "rgba(239, 68, 68, 0.94)",
  },
  linkInfoArrowFill: {
    cssVar: "--ui-canvas-link-info-arrow-fill",
    fallback: "rgba(2, 132, 199, 0.98)",
  },
  linkInfoStroke: { cssVar: "--ui-canvas-link-info-stroke", fallback: "rgba(14, 165, 233, 0.9)" },
  linkLabelDangerFill: {
    cssVar: "--ui-canvas-link-label-danger-fill",
    fallback: "rgba(254, 242, 242, 0.98)",
  },
  linkLabelDangerStroke: {
    cssVar: "--ui-canvas-link-label-danger-stroke",
    fallback: "rgba(244, 63, 94, 0.44)",
  },
  linkLabelDangerText: {
    cssVar: "--ui-canvas-link-label-danger-text",
    fallback: "rgba(159, 18, 57, 0.98)",
  },
  linkLabelDefaultFill: {
    cssVar: "--ui-canvas-link-label-default-fill",
    fallback: "rgba(255, 255, 255, 0.96)",
  },
  linkLabelDefaultStroke: {
    cssVar: "--ui-canvas-link-label-default-stroke",
    fallback: "rgba(100, 116, 139, 0.32)",
  },
  linkLabelDefaultText: {
    cssVar: "--ui-canvas-link-label-default-text",
    fallback: "rgba(30, 41, 59, 0.9)",
  },
  linkLabelInfoFill: {
    cssVar: "--ui-canvas-link-label-info-fill",
    fallback: "rgba(240, 249, 255, 0.98)",
  },
  linkLabelInfoStroke: {
    cssVar: "--ui-canvas-link-label-info-stroke",
    fallback: "rgba(14, 165, 233, 0.42)",
  },
  linkLabelInfoText: {
    cssVar: "--ui-canvas-link-label-info-text",
    fallback: "rgba(3, 105, 161, 0.98)",
  },
  linkLabelPanelShadow: {
    cssVar: "--ui-canvas-link-label-panel-shadow",
    fallback: "rgba(15, 23, 42, 0.1)",
  },
  linkLabelSuccessFill: {
    cssVar: "--ui-canvas-link-label-success-fill",
    fallback: "rgba(236, 253, 245, 0.97)",
  },
  linkLabelSuccessStroke: {
    cssVar: "--ui-canvas-link-label-success-stroke",
    fallback: "rgba(20, 184, 166, 0.42)",
  },
  linkLabelSuccessText: {
    cssVar: "--ui-canvas-link-label-success-text",
    fallback: "rgba(15, 118, 110, 0.98)",
  },
  linkLabelWarningFill: {
    cssVar: "--ui-canvas-link-label-warning-fill",
    fallback: "rgba(255, 251, 235, 0.98)",
  },
  linkLabelWarningStroke: {
    cssVar: "--ui-canvas-link-label-warning-stroke",
    fallback: "rgba(245, 158, 11, 0.46)",
  },
  linkLabelWarningText: {
    cssVar: "--ui-canvas-link-label-warning-text",
    fallback: "rgba(146, 64, 14, 0.98)",
  },
  linkPreviewArrowFill: {
    cssVar: "--ui-canvas-link-preview-arrow-fill",
    fallback: "rgba(109, 40, 217, 0.96)",
  },
  linkPreviewStroke: {
    cssVar: "--ui-canvas-link-preview-stroke",
    fallback: "rgba(124, 58, 237, 0.92)",
  },
  linkSuccessArrowFill: {
    cssVar: "--ui-canvas-link-success-arrow-fill",
    fallback: "rgba(15, 118, 110, 0.96)",
  },
  linkSuccessStroke: {
    cssVar: "--ui-canvas-link-success-stroke",
    fallback: "rgba(20, 184, 166, 0.9)",
  },
  linkUserAuthoredArrowFill: {
    cssVar: "--ui-canvas-link-user-authored-arrow-fill",
    fallback: "rgba(14, 165, 233, 0.88)",
  },
  linkUserAuthoredStroke: {
    cssVar: "--ui-canvas-link-user-authored-stroke",
    fallback: "rgba(14, 165, 233, 0.82)",
  },
  linkWarningArrowFill: {
    cssVar: "--ui-canvas-link-warning-arrow-fill",
    fallback: "rgba(217, 119, 6, 0.98)",
  },
  linkWarningStroke: {
    cssVar: "--ui-canvas-link-warning-stroke",
    fallback: "rgba(245, 158, 11, 0.94)",
  },
  ...
  mediaPreviewCaptionText: {
    cssVar: "--ui-canvas-media-preview-caption-text",
    fallback: "rgba(15, 23, 42, 0.76)",
  },
  mediaPreviewPanelFill: {
    cssVar: "--ui-canvas-media-preview-panel-fill",
    fallback: "rgba(241, 245, 249, 0.95)",
  },
  mediaPreviewPanelStroke: {
    cssVar: "--ui-canvas-media-preview-panel-stroke",
    fallback: "rgba(148, 163, 184, 0.22)",
  },
  portAccentAccentColor: { cssVar: "--ui-canvas-port-accent-accent-color", fallback: "#3b82f6" },
  portAccentAnchor: {
    cssVar: "--ui-canvas-port-accent-anchor",
    fallback: "rgba(59, 130, 246, 0.94)",
  },
  portAccentFill: {
    cssVar: "--ui-canvas-port-accent-fill",
    fallback: "rgba(59, 130, 246, 0.18)",
  },
  portAccentHalo: {
    cssVar: "--ui-canvas-port-accent-halo",
    fallback: "rgba(59, 130, 246, 0.18)",
  },
  portAccentLine: {
    cssVar: "--ui-canvas-port-accent-line",
    fallback: "rgba(59, 130, 246, 0.88)",
  },
  portAccentStroke: {
    cssVar: "--ui-canvas-port-accent-stroke",
    fallback: "rgba(37, 99, 235, 0.48)",
  },
  portAccentText: {
    cssVar: "--ui-canvas-port-accent-text",
    fallback: "rgba(219, 234, 254, 0.98)",
  },
  portDangerAccentColor: { cssVar: "--ui-canvas-port-danger-accent-color", fallback: "#ef4444" },
  portDangerAnchor: {
    cssVar: "--ui-canvas-port-danger-anchor",
    fallback: "rgba(239, 68, 68, 0.92)",
  },
  ...
  portDangerStroke: {
    cssVar: "--ui-canvas-port-danger-stroke",
    fallback: "rgba(220, 38, 38, 0.48)",
  },
  portDangerText: {
    cssVar: "--ui-canvas-port-danger-text",
    fallback: "rgba(254, 226, 226, 0.98)",
  },
  portDefaultAnchor: {
    cssVar: "--ui-canvas-port-default-anchor",
    fallback: "rgba(71, 85, 105, 0.88)",
  },
  portDefaultFill: {
    cssVar: "--ui-canvas-port-default-fill",
    fallback: "rgba(255, 255, 255, 0.94)",
  },
  portDefaultHalo: {
    cssVar: "--ui-canvas-port-default-halo",
    fallback: "rgba(148, 163, 184, 0.16)",
  },
  portDefaultLine: {
    cssVar: "--ui-canvas-port-default-line",
    fallback: "rgba(71, 85, 105, 0.82)",
  },
  portDefaultStroke: {
    cssVar: "--ui-canvas-port-default-stroke",
    fallback: "rgba(148, 163, 184, 0.34)",
  },
  portDefaultText: {
    cssVar: "--ui-canvas-port-default-text",
    fallback: "rgba(71, 85, 105, 0.96)",
  },
  portSuccessAccentColor: {
    cssVar: "--ui-canvas-port-success-accent-color",
    fallback: "#22c55e",
  },
  portSuccessAnchor: {
    cssVar: "--ui-canvas-port-success-anchor",
    fallback: "rgba(34, 197, 94, 0.92)",
  },
  portSuccessFill: {
    cssVar: "--ui-canvas-port-success-fill",
    fallback: "rgba(34, 197, 94, 0.18)",
  },
  portSuccessHalo: {
    cssVar: "--ui-canvas-port-success-halo",
    fallback: "rgba(34, 197, 94, 0.18)",
  },
  portSuccessLine: {
    cssVar: "--ui-canvas-port-success-line",
    fallback: "rgba(34, 197, 94, 0.86)",
  },
  portSuccessStroke: {
    cssVar: "--ui-canvas-port-success-stroke",
    fallback: "rgba(22, 163, 74, 0.48)",
  },
  portSuccessText: {
    cssVar: "--ui-canvas-port-success-text",
    fallback: "rgba(220, 252, 231, 0.98)",
  },
  portWarningAccentColor: {
    cssVar: "--ui-canvas-port-warning-accent-color",
    fallback: "#f59e0b",
  },
  portWarningAnchor: {
    cssVar: "--ui-canvas-port-warning-anchor",
    fallback: "rgba(245, 158, 11, 0.92)",
  },
  portWarningFill: {
    cssVar: "--ui-canvas-port-warning-fill",
    fallback: "rgba(245, 158, 11, 0.18)",
  },
  portWarningHalo: {
    cssVar: "--ui-canvas-port-warning-halo",
    fallback: "rgba(245, 158, 11, 0.18)",
  },
  portWarningLine: {
    cssVar: "--ui-canvas-port-warning-line",
    fallback: "rgba(245, 158, 11, 0.88)",
  },
  portWarningStroke: {
    cssVar: "--ui-canvas-port-warning-stroke",
    fallback: "rgba(217, 119, 6, 0.48)",
  },
  portWarningText: {
    cssVar: "--ui-canvas-port-warning-text",
    fallback: "rgba(254, 243, 199, 0.98)",
  },
  progressCompleteArc: {
    cssVar: "--ui-canvas-progress-complete-arc",
    fallback: "rgba(5, 150, 105, 0.92)",
  },
  progressInProgressArc: {
    cssVar: "--ui-canvas-progress-in-progress-arc",
    fallback: "rgba(124, 58, 237, 0.92)",
  },
  progressTextDefault: {
    cssVar: "--ui-canvas-progress-text-default",
    fallback: "rgba(15, 23, 42, 0.78)",
  },
  progressTrackDefault: {
    cssVar: "--ui-canvas-progress-track-default",
    fallback: "rgba(148, 163, 184, 0.28)",
  },
};

export function resolveCanvasColors(host) {
  return themeTokens.readTokens(host, canvasColorTokenMap);
}
