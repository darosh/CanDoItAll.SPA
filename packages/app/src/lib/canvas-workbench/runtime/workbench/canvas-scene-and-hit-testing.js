import { showPopover, renderConnectorAnchorOverlay } from "./runtime-rendering.js";
import {
  clamp,
  round,
  getNodeSize,
...
import { hidePopover, getHostPoint, worldToHostPoint } from "./layout-and-legacy-render.js";

export function getCanvasRuntimePrimitives() {
  return window.ZyCanvasPrimitives || null;
}

export function createFallbackHitRegistry() {
  return {
    items: [],
    clear() {
      this.items = [];
...
export function createCanvasHitRegistry() {
  const primitives = getCanvasRuntimePrimitives();
  return primitives?.HitRegistry ? new primitives.HitRegistry() : createFallbackHitRegistry();
}

export function createCanvasSurfaceHost(canvas, resizeTarget) {
  const primitives = getCanvasRuntimePrimitives();
...
export function destroyCanvasSurfaceHost(surface) {
  surface?.destroy?.();
}
...
function buildAccentPalette(accent, isSelected) {
  ...
  const whiteMix = (ratio) =>
    mixColorChannels(accentChannels, { red: 255, green: 255, blue: 255 }, ratio);
  const darkMix = (ratio) =>
    mixColorChannels(accentChannels, { red: 15, green: 23, blue: 42 }, ratio);
...
export function resolveNodeAccentColor(node, colors) {
...
export function resolveCanvasNodePaletteStyle(node, accent, isSelected, colors) {
  const resolved = colors || {};
  if (node?.isReadOnly) {
    return {
      surfaceFill: resolved.nodeReadonlySurfaceFill || "rgba(248, 250, 252, 0.98)",
      surfaceStroke: isSelected
        ? resolved.nodeReadonlySurfaceStrokeSelected || "rgba(71, 85, 105, 0.9)"
        : resolved.nodeReadonlySurfaceStrokeUnselected || "rgba(148, 163, 184, 0.34)",
  ...
  const paletteKey = (
    node?.paletteKey || (node?.family === "root" ? "primary" : "neutral")
  ).toLowerCase();
  const palettes = {
    primary: {
      surfaceFill: resolved.nodePrimarySurfaceFill || "rgba(15, 23, 42, 0.98)",
      surfaceStroke: isSelected
        ? resolved.nodePrimarySurfaceStrokeSelected || "rgba(248, 250, 252, 0.94)"
        : resolved.nodePrimarySurfaceStrokeUnselected || "rgba(51, 65, 85, 0.96)",
    ...
    secondary: {
      ...
      surfaceStroke: isSelected
        ? resolved.nodeSecondarySurfaceStrokeSelected || "rgba(124, 58, 237, 0.92)"
        : resolved.nodeSecondarySurfaceStrokeUnselected || "rgba(167, 139, 250, 0.62)",
    ...
    success: {
      ...
      surfaceStroke: isSelected
        ? resolved.nodeSuccessSurfaceStrokeSelected || "rgba(22, 163, 74, 0.92)"
        : resolved.nodeSuccessSurfaceStrokeUnselected || "rgba(74, 222, 128, 0.62)",
    ...
    info: {
      ...
      surfaceStroke: isSelected
        ? resolved.nodeInfoSurfaceStrokeSelected || "rgba(2, 132, 199, 0.92)"
        : resolved.nodeInfoSurfaceStrokeUnselected || "rgba(125, 211, 252, 0.64)",
    ...
    warning: {
      ...
      surfaceStroke: isSelected
        ? resolved.nodeWarningSurfaceStrokeSelected || "rgba(217, 119, 6, 0.92)"
        : resolved.nodeWarningSurfaceStrokeUnselected || "rgba(251, 191, 36, 0.64)",
    ...
    danger: {
      ...
      surfaceStroke: isSelected
        ? resolved.nodeDangerSurfaceStrokeSelected || "rgba(220, 38, 38, 0.94)"
        : resolved.nodeDangerSurfaceStrokeUnselected || "rgba(252, 165, 165, 0.7)",
    ...
    "workflow-decision": {
      ...
      surfaceStroke: isSelected
        ? resolved.nodeWorkflowDecisionSurfaceStrokeSelected || "rgba(15, 118, 110, 0.96)"
        : resolved.nodeWorkflowDecisionSurfaceStrokeUnselected || "rgba(20, 184, 166, 0.64)",
    ...
    neutral: {
      ...
      surfaceStroke: isSelected
        ? resolved.nodeNeutralSurfaceStrokeSelected || "rgba(71, 85, 105, 0.92)"
        : resolved.nodeNeutralSurfaceStrokeUnselected || "rgba(148, 163, 184, 0.44)",
...
export function resolveAnchorRect(anchor) {
  if (!anchor) {
    return null;
  }
...
export function buildRect(left, top, width, height) {
  return {
    left,
    top,
    width,
...
export function boundsToHitRect(bounds) {
  return {
    x: round(bounds.left),
    y: round(bounds.top),
...
export function projectSceneBounds(state, bounds) {
...
export function getNodeSceneBounds(state, node) {
...
export function clearSceneHotZones(state) {
  state.sceneHitRegistry?.clear?.();
  state.sceneHotZones = [];
}

export function registerSceneHotZone(state, bounds, metadata) {
...
export function getSceneHitAtPoint(state, pointX, pointY) {
  return state.sceneHitRegistry?.find?.(pointX, pointY) || null;
}

export function getSceneHitAtEvent(state, event) {
  const point = getHostPoint(state, event.clientX, event.clientY);
  return getSceneHitAtPoint(state, point.x, point.y);
}

export function resolveHitNode(state, hitTarget) {
  return hitTarget?.nodeId ? state.lookups.byId.get(hitTarget.nodeId) || null : null;
}

export function clearScenePopoverHover(state) {
...
function showScenePopover(state, anchorRect, annotation) {
  return showPopover(state, anchorRect, annotation) !== false;
}
...
export function syncSceneHoverState(state, event) {
  ...
  if ((state.hoveredNodeId || null) !== nextNodeId) {
    ...
    renderConnectorAnchors(
      state,
      Array.isArray(visibleNodes) ? getProjectedNodes(state, visibleNodes) : [],
    );
  }
