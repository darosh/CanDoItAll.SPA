...
function createResizeObserver(callback) {
  return typeof window.ResizeObserver === "function" ? new window.ResizeObserver(callback) : null;
}
...
function resolveContainer(host, options) {
  ...
  return (
    host.closest("[data-cda-overlay-container], .cw-stage-surface, .wgl-workbench-stage") ||
    host.parentElement
  );
}
...
function resolveSafeTopElement(container, options) {
  ...
  const frame = container.closest(
    "[data-cda-overlay-frame], .cw-workbench-frame, .wgl-workbench-shell",
  );
  if (frame) {
    const frameMatch = frame.querySelector(
      "[data-cda-overlay-safe-top], .cw-toolbar, .wgl-overlay-toolbar, .wgl-workbench-toolbar",
    );
    if (frameMatch) {
      return frameMatch;
    }
...
function hasCustomGeometry(windowState) {
  return !!(
    windowState &&
    (windowState.left > 0 || windowState.top > 0 || windowState.width > 0 || windowState.height > 0)
  );
}
...
function syncWindowObservers(state) {
  ...
  state.resizeObserver = createResizeObserver(() => {
    if (
      ...
      !state.container ||
      state.host.classList.contains("is-dragging")
    ) {
      return;
    }
...
function createLegacyState(panel) {
  ...
  state.pointerMove = (event) => {
    ...
    const maxLeft = Math.max(
      margin,
      state.container.clientWidth - state.panel.offsetWidth - margin,
    );
    const maxTop = Math.max(
      safeTop,
      state.container.clientHeight - state.panel.offsetHeight - margin,
    );
...
export function create(host, dotNetRef, options) {
  if (!host) {
    return;
  }
...
export { mountLegacy, resetLegacy };
