...
export function normalize(selectedNodeIds, primaryNodeId) {
...
export function replace(selectedNodeIds, primaryNodeId) {
  return normalize(selectedNodeIds, primaryNodeId);
}

export function selectOne(nodeId) {
  return normalize([nodeId], nodeId);
}

export function toggle(selectedNodeIds, nodeId, primaryNodeId) {
  const ordered = uniqueIds(selectedNodeIds);
  ...
  if (existingIndex >= 0) {
    ...
    return normalize(
      ordered,
      normalizeId(primaryNodeId) === normalizedNodeId ? null : primaryNodeId,
    );
  }
...
export function removeMissing(selectedNodeIds, validNodeIds, primaryNodeId) {
  const valid = new Set(uniqueIds(validNodeIds));
  const filtered = uniqueIds(selectedNodeIds).filter((nodeId) => valid.has(nodeId));
  const normalizedPrimary = normalizeId(primaryNodeId);
  return normalize(
    filtered,
    normalizedPrimary && valid.has(normalizedPrimary) ? normalizedPrimary : null,
  );
}
