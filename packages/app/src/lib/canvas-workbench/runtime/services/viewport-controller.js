...
export function clampPanToScene(request) {
...
export function createFitViewTarget(request) {
  const bounds = normalizeBounds(request?.bounds);
  const hostWidth = normalizeNumber(request?.hostWidth, 0);
  const hostHeight = normalizeNumber(request?.hostHeight, 0);
  if (!bounds || hostWidth <= 0 || hostHeight <= 0) {
  ...
  const zoom = clamp(
    Math.min((hostWidth - padding) / width, (hostHeight - padding) / height),
    defaults.minZoom,
    defaults.maxZoom,
  );
...
export function createFocusTarget(request) {
  const bounds = normalizeBounds(request?.bounds);
  const hostWidth = normalizeNumber(request?.hostWidth, 0);
  const hostHeight = normalizeNumber(request?.hostHeight, 0);
...
export function hostToScene(request) {
...
export function sceneToHost(request) {
  const state = normalizeUiState(request);
  return {
...
export function zoomAroundPoint(request) {
