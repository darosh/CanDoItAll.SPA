...
function normalizeFontSpec(spec) {
  ...
  const cssFont =
    typeof raw.cssFont === "string" && raw.cssFont.trim().length > 0 ? raw.cssFont.trim() : null;
  ...
  const family =
    typeof raw.family === "string" && raw.family.trim().length > 0
      ? raw.family.trim()
      : '"DM Sans", sans-serif';
  const lineHeightPx = asNumber(raw.lineHeightPx, Math.max(14, sizePx * 1.35));
  const letterSpacingPx = asNumber(raw.letterSpacingPx, 0);
  const normalizedCssFont =
    cssFont || `${weight} ${sizePx.toFixed(2).replace(/\.00$/, "")}px ${family}`;
...
export function measure(request) {
  ...
  const truncationMode =
    typeof normalized.truncationMode === "string"
      ? normalized.truncationMode.trim().toLowerCase()
      : "ellipsis";
  const ellipsis =
    truncationMode === "clip"
      ? ""
      : typeof normalized.ellipsis === "string" && normalized.ellipsis.length > 0
        ? normalized.ellipsis
        : "...";
  ...
  for (; tokenIndex < tokens.length; tokenIndex += 1) {
    ...
    currentLine = truncationMode === "clip" ? token : fitToWidth(token, maxWidth, font, ellipsis);
  ...
  const result = {
    ...
    estimatedWidth:
      lineEntries.length === 0 ? 0 : Math.max(...lineEntries.map((line) => line.estimatedWidth)),
...
export function fitElementText(element, options) {
  if (!element) {
    return null;
  }
  ...
  const minFontSize = Math.min(
    initialFontSize,
    asNumber(settings.minFontSize, Math.max(8, initialFontSize - 3)),
  );
  ...
  const fullText = normalizeText(
    settings.text || settings.fullText || element.dataset.fullText || element.textContent,
  );
  ...
  do {
      ...
      font: {
        ...
        lineHeightPx:
          computed.lineHeight === "normal"
            ? fontSize * 1.25
            : asNumber(computed.lineHeight, fontSize * 1.25),
    ...
    if (
      ...
      (result.estimatedWidth <= maxWidth + 0.5 && result.estimatedHeight <= maxHeight + 0.5)
    ) {
      break;
    }
